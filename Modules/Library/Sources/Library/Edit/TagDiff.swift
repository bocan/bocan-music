...
public enum TagDiff {
    ...
    public static func diff(before: TrackTags, after: TrackTags) -> TrackTagPatch {
        ...
        if before.artist != after.artist {
            patch.artist = after.artist
        }
        if before.albumArtist != after.albumArtist {
            patch.albumArtist = after.albumArtist
        }
        if before.album != after.album {
            patch.album = after.album
        }
        if before.genre != after.genre {
            patch.genre = after.genre
        }
        if before.composer != after.composer {
            patch.composer = after.composer
        }
        ...
        if before.sortArtist != after.sortArtist {
            patch.sortArtist = after.sortArtist
        }
        ...
        if before.trackTotal != after.trackTotal {
            patch.trackTotal = after.trackTotal
        }
        if before.discNumber != after.discNumber {
            patch.discNumber = after.discNumber
        }
        if before.discTotal != after.discTotal {
            patch.discTotal = after.discTotal
        }

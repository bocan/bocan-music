...
- **`upsertByFeedURL` preserves user-owned podcast fields** (`id`, `addedAt`, `subscribed`, `autoDownload`, `sortIndex`, `playbackSpeed`, `episodeSort`, `retentionLimit`) and refreshes everything else from the feed (including the feed-derived `showType`). When you add a user-owned column, add it to that preservation copy in `PodcastRepository` (in `Persistence`).

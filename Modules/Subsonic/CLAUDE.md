...
- **`Scrobble` and `Playback` must not import `Subsonic`; `UI` may.** The charter's dependency table (`docs/design-spec/_standards.md`, "Module layout") lists `Subsonic` in UI's row, and UI uses this module's value types, its browse data source and, in Settings, the service itself. `Scrobble` and `Playback` sit beside this module in the DAG, so they declare protocols (scrobble delivery, stream resolving) and the **App layer** provides the concrete adapters. UI also keeps protocol seams where a view model only needs a narrow slice (sidebar listing, connection observing, capability-change observing, metadata caching); prefer widening one of those to handing a view model the whole service.

...
- **Localization (issue #314).** SwiftPM only copies `Resources/Localizable.xcstrings`; **only the Xcode build compiles it** into `.strings`/`.stringsdict`. A bare `Text("…")` in this SPM module resolves against `Bundle.main`, not the module catalog, so user-facing copy must go through the `L10n` helper (`Text(localized:)` / `L10n.string(_:)`, which pass `bundle: .module`). The `no_bare_user_facing_literal` lint guard enforces this module-wide. Tests validate the catalog *content* (deterministic) rather than runtime resolution. The conversion recipe is documented in `docs/design-spec/localization.md`. Note: an Xcode build (`make test`) rewrites the catalog via auto-extraction; that churn is unrelated to most changes and can be reverted.

...
public final class TagEditorViewModel: ObservableObject {
    ...
    // MARK: - Field helpers: TagEditorViewModel+Fields.swift

    /// Removes the cover art: clears pending data and marks the art as deleted.
    /// Stays in this file because the two flags it writes are `private(set)`.
    ...
    private func buildPatch() -> TrackTagPatch {
        ...
        if self.isSingleTrack {
            if case let .edited(val) = self.trackNumber {
                patch.trackNumber = val
            }
            if case let .edited(val) = self.trackTotal {
                patch.trackTotal = val
            }
        }
        ...
        if case let .edited(val) = self.loved,
           self.isSingleTrack || self.enabledFields.contains(.loved) {
            patch.loved = val
        }

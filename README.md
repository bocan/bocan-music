...
- **Deep Dive.** Get Info on an artist (new: right-click any artist) or a track gains a Deep Dive tab: a concise report from MusicBrainz and Wikipedia with the artist's bio, current and past members with their years and instruments, a discography by year with the albums you own ticked, external links, and for a track the recording's ISRC, first release and every release it appears on. Reports are cached on disk for a week and shown stale when offline; an artist without a MusicBrainz id in its tags is matched by name and flagged as a guess.

...
- **Internet radio** - a first-class station catalog in the sidebar. Add stations by hand, paste a `.pls`/`.m3u` playlist URL into the Add Station sheet and every stream inside it is offered as a station, or import/drop a playlist file and its stream URLs become stations instead of missing tracks. Live streams play through the FFmpeg decoder with automatic reconnect, the scrubber stays honest about live audio having no timeline, and station idents never pollute your scrobbles or play history.

...
- **Library Summary** : open **Tools -> Library Summary** (`⌘⇧Y`) for whole-library totals at a glance: songs, albums, artists, album artists, and total playback duration (in days, hours, minutes, and seconds). The **Library Hygiene** tab hunts down the problems worth fixing: track-number gaps (you have 1, 2, 3, 5, 6), albums exploded into one-track shards by inconsistent album-artist tags, implausible or contradictory years, albums missing artwork, year, or MusicBrainz IDs, and tracks whose files have vanished from disk. The **Audio Quality** tab knows what your library is made of: codec, sample-rate, bit-depth, and bitrate distributions, the lossless-to-lossy split by song count and by gigabytes (they tell different stories), mixed-format albums (nine FLACs and that one MP3 from 2011), and tracks whose EBU R128 true peak clips past full scale. And **Tools -> Analyse Provenance** hunts down "FLACs" laundered from lossy sources: it decodes sample windows from every lossless file, looks for the hard spectral shelf a lossy encoder leaves behind (roughly 16 kHz for 128k MP3, 20 kHz for 320k), and records a confidence-weighted verdict. Suspected, never accused: live recordings and old digitisations can false-positive, so nothing is deleted or retagged, verdicts persist across launches, and a changed file is automatically re-queued for analysis. The batch runs one file at a time at low priority so playback never stutters, with live progress and cancel in the Audio Quality tab, and an **Analyse Now** button lives right there too: nothing runs behind your back, so the button shows how many lossless files are waiting (new and changed files re-queue automatically) and goes quiet when coverage is complete. Every flagged file lands in a **Suspected Transcodes** list: highest confidence first, each row reading like "87% confident · shelf at 16 kHz", one click from its album, with the not-yet-analysed count alongside so the coverage is honest. The **Collection Shape** tab charts when your music was made and how you actually treat it: a release-year histogram, ownership versus listening laid out per decade as two aligned strips (the gap between what you own and what you play is the interesting bit), the one-track guest long tail against your ten-plus-album lifers and deepest catalogues, the longest and shortest songs and the longest album (one click from each), and average album length by decade, which climbs when the CD arrives and falls again after. And the **Listening Behaviour** tab starts with your history itself: **Tools -> Import Last.fm History** (or the button right in the tab) backfills years of scrobbles from the official Last.fm export. Plays are matched to your library by MusicBrainz ID first, then by artist and title; music you no longer own is kept unmatched rather than thrown away, and **Match Again** links it up as your library grows. Imported plays never touch your local play counts (and echoes of plays Bòcan itself scrobbled are deduplicated), the whole import is removable in one click, and re-importing a fresh export next year only adds what's new. On top of that history the tab answers the uncomfortable questions: how much of your library you've actually ever played, and a **Gini coefficient** over play counts to quantify how brutally you rotate the same few hundred songs (most people are north of 0.7 and horrified to learn it). **Skip Candidates** is the delete list: tracks skipped more often than played, with the average bail-out point ("5 skips vs 1 play, bails around 0:42"), and since a play only counts past half the song, the numbers mean it. **Dormant Favourites** finds what you loved and then forgot: heavy lifetime plays, silent for two years. And **Abandoned Albums** is the "you never got past the singles" report. **When You Listen** turns every timestamp, local and imported, into an hour-by-weekday heatmap of your habits (rendered in your current time zone, and the footer admits it). The **Discovery Rate** chart plots artists heard for the first time each month across your entire history, unmatched scrobbles included; watch it flatline and make peace with that. And **Seasonal Listening** names the artists whose plays pile into one month of the year across multiple years, so your December self can be formally identified. Finally, the **Podcasts** tab audits the subscription habit: your unheard backlog in hours with a projection against your actual listening rate ("about 19 months", or the unflinching "at your current rate: never"), dead feeds that haven't published in six months but still hold a sidebar seat, the downloaded-but-never-started hoard per show with its disk bill, and reapable storage: episodes you finished months ago that are still on disk. All six tabs are live.

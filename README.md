...
- **Real-time visualisers**: spectrum bars, oscilloscope, Halo (a breathing spectrum ring with beat ripples), Cascade (a scrolling spectrogram waterfall), Starfield (a frequency-coloured warp field that stretches into streaks on the beat), and Nebula (a luminous gas cloud whose churn, drifting wisps, and pressure waves all follow the music), dockable or full-screen with `⌘⇧F`. All six render on the GPU through Metal, with six colour palettes including a music-steered Drift and a magnitude-to-heat Thermal ramp.

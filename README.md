...
- **Library Summary** : open **Tools -> Library Summary** (`⌘⇧Y`) for whole-library totals at a glance: songs, albums, artists, album artists, and total playback duration (in days, hours, minutes, and seconds). More analytics tabs (library hygiene, audio quality, collection shape, listening behaviour, podcasts) are on the roadmap.

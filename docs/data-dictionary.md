...
Rows reviewed: 50. Rows with only a migration reference: 243.
...
| `track_id` | INTEGER | ConflictResolver.swift, ContentHashService.swift, CueMarkerService.swift, DSPAssignmentRepository.swift, DSPViewModel.swift (+57) | DockTileController.swift, DuplicateReviewSheet.swift, FieldDefinitions.swift, GaplessScheduler.swift, LibraryStatsRepository+ListeningBehaviour.swift (+9) | M001 |  |
...
| column | type | written by | read by | traces to | notes |
|---|---|---|---|---|---|
| `track_id` (pk) | INTEGER | ConflictResolver.swift, ContentHashService.swift, CueMarkerService.swift, DSPAssignmentRepository.swift, DSPViewModel.swift (+57) | DockTileController.swift, DuplicateReviewSheet.swift, FieldDefinitions.swift, GaplessScheduler.swift, LibraryStatsRepository+ListeningBehaviour.swift (+9) | M001 |  |
...
| `content_hash` | TEXT | EpisodeStateRepository.swift, TrackImporter.swift, TrackRepository.swift | ContentHashService.swift, DownloadStore.swift, EpisodeDownloadManager.swift, FileServing.swift, ManifestBuilder.swift (+1) | M001 |  |
...
Record: `SyncTranscode.swift`

| column | type | written by | read by | traces to | notes |
|---|---|---|---|---|---|
| `track_id` (pk) | INTEGER | ConflictResolver.swift, ContentHashService.swift, CueMarkerService.swift, DSPAssignmentRepository.swift, DSPViewModel.swift (+57) | DockTileController.swift, DuplicateReviewSheet.swift, FieldDefinitions.swift, GaplessScheduler.swift, LibraryStatsRepository+ListeningBehaviour.swift (+9) | ADR-088 | Track the artifact belongs to; cascades away with the track. |
...
| `source_content_hash` | TEXT | SyncTranscodeRepository.swift |  | ADR-088 | The track's content_hash the artifact was derived from; the row is valid only while it still matches, so a retag invalidates the artifact. |
...
| `size` | INTEGER | n/a (common identifier, see notes) | n/a | ADR-088 | Artifact size in bytes, advertised as the manifest size; feeds the phone's storage accounting. |
| `bitrate` | INTEGER | n/a (common identifier, see notes) | n/a | ADR-088 | Artifact bitrate in kbps when known; advertised as the manifest bitrate. NULL when the encoder did not report one. |
| `created_at` | INTEGER | BackupRing.swift, PlaylistService.swift, SmartPlaylistService.swift, SubsonicServer.swift, SubsonicServerRepository.swift (+1) |  | ADR-088 | Unix seconds when the artifact was encoded. |
| `served_at` | INTEGER | SyncTranscodeRepository.swift |  | ADR-088 | Unix seconds when a file response last delivered the artifact through EOF; NULL until then. The prepare-and-release sweep deletes bytes for stamped rows unless the keep toggle is on. |
...
| `content_hash` | TEXT | EpisodeStateRepository.swift, TrackImporter.swift, TrackRepository.swift | ContentHashService.swift, DownloadStore.swift, EpisodeDownloadManager.swift, FileServing.swift, ManifestBuilder.swift (+1) | ADR-065, #423 | Sync ETag from ContentHashService; kept on rescan while file_mtime is unchanged, cleared when the file changed (same rule as provenance). |

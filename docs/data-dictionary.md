...
Rows reviewed: 51. Rows with only a migration reference: 242.
...
|---|---|---|---|---|---|
| `album_id` (pk) | INTEGER | A11yIdentifiers.swift, AlbumDetailView.swift, AlbumRepository.swift, AlbumsGridView.swift, AlbumsViewModel.swift (+37) | AlbumRepository+CollectionCards.swift, Artwork.swift, FieldDefinitions.swift, IdentifyTrackViewModel.swift, LibraryStatsRepository.swift (+12) | M001 |  |
...
| `album_artist_id` | INTEGER | AlbumRepository.swift, AlbumsViewModel.swift, ArtistRepository.swift, EditTransaction.swift, LibraryStatsRepository+Hygiene.swift (+4) | AlbumDetailView.swift, AlbumRepository+CollectionCards.swift, AlbumsGridView.swift, BatchCoverArtViewModel.swift, DeepDiveService.swift (+10) | M001 |  |
...
| `cover_art_hash` | TEXT | AlbumRepository.swift, EditTransaction.swift, MetadataEditService.swift, TrackImporter.swift | AlbumDetailView.swift, Artwork.swift, BackupRing.swift, BatchCoverArtViewModel.swift, FieldDefinitions.swift (+4) | M001 |  |
...
| `track_id` | INTEGER | ConflictResolver.swift, ContentHashService.swift, CueMarkerService.swift, DSPAssignmentRepository.swift, DSPViewModel.swift (+60) | DockTileController.swift, DuplicateReviewSheet.swift, FieldDefinitions.swift, GaplessScheduler.swift, LibraryStatsRepository+ListeningBehaviour.swift (+10) | M001 |  |
...
| column | type | written by | read by | traces to | notes |
|---|---|---|---|---|---|
| `track_id` (pk) | INTEGER | ConflictResolver.swift, ContentHashService.swift, CueMarkerService.swift, DSPAssignmentRepository.swift, DSPViewModel.swift (+60) | DockTileController.swift, DuplicateReviewSheet.swift, FieldDefinitions.swift, GaplessScheduler.swift, LibraryStatsRepository+ListeningBehaviour.swift (+10) | M001 |  |
...
| `content_hash` | TEXT | EpisodeStateRepository.swift, TrackImporter.swift, TrackRepository.swift | ContentHashService.swift, DownloadStore.swift, EpisodeDownloadManager.swift, FileServing.swift, ManifestBuilder.swift (+2) | M001 |  |
...
|---|---|---|---|---|---|
| `id` (pk) | INTEGER | n/a (common identifier, see notes) | n/a | M001 |  |
| `profile_json` | BLOB | SyncProfileRepository.swift | ManifestRoutes.swift, TranscodeCoordinator.swift | M031 |  |
...
| column | type | written by | read by | traces to | notes |
|---|---|---|---|---|---|
| `track_id` (pk) | INTEGER | ConflictResolver.swift, ContentHashService.swift, CueMarkerService.swift, DSPAssignmentRepository.swift, DSPViewModel.swift (+60) | DockTileController.swift, DuplicateReviewSheet.swift, FieldDefinitions.swift, GaplessScheduler.swift, LibraryStatsRepository+ListeningBehaviour.swift (+10) | ADR-088 | Track the artifact belongs to; cascades away with the track. |
...
| `created_at` | INTEGER | BackupRing.swift, PlaylistService.swift, SmartPlaylistService.swift, SubsonicServer.swift, SubsonicServerRepository.swift (+2) |  | ADR-088 | Unix seconds when the artifact was encoded. |
| `served_at` | INTEGER | SyncTranscodeRepository.swift | FileServing.swift, ManifestBuilder.swift, TranscodeCoordinator.swift | ADR-088 | Unix seconds when a file response last delivered the artifact through EOF; NULL until then. The prepare-and-release sweep deletes bytes for stamped rows unless the keep toggle is on. |
...
|---|---|---|---|---|---|
| `id` (pk) | INTEGER | n/a (common identifier, see notes) | n/a | M001 |  |
| `file_url` | TEXT | BackupRing.swift, CueMarkerService.swift, EditTransaction.swift, Fingerprinter.swift, LibraryScanner.swift (+16) | ArtworkEditor.swift, BatchCoverArtViewModel.swift, ContentHashService.swift, CoverArtCache.swift, DeepDiveCache.swift (+23) | M001 |  |
...
| `duration` | REAL | n/a (common identifier, see notes) | n/a | M001 |  |
| `sample_rate` | INTEGER | AVFoundationProperties.swift, AudioEngine+GaplessAPI.swift, AudioEngine.swift, AudioTap.swift, AudioTime.swift (+21) | A11yIdentifiers.swift, AVFoundationDecoder.swift, AppLogger.swift, AudioTranscoder+Pipeline.swift, BufferPump.swift (+14) | M001 |  |
...
| `channel_count` | INTEGER | FFmpegDecoder+StreamDetails.swift, LibraryViewModel+Radio.swift, LibraryViewModel+Subsonic.swift, Manifest.swift, ManifestBuilder.swift (+5) | AVFoundationProperties.swift, AudioTap.swift, BufferPump.swift, DecoderFactory.swift, FormatBridge.swift (+7) | ADR-091, #513 | Channel count from the tag reader: TagLib, or AVFoundation for a raw Dolby file TagLib cannot parse. Read by TrackInfoPanel.swift and TagEditorSheet+InfoTabs.swift, which show it as a layout name (Mono, Stereo, 5.1, 7.1, else a count) with hover text saying a surround mix plays folded to stereo. Playback folds every source channel into stereo, so the value describes the file, not what is heard. |
| `is_lossless` | BOOLEAN | Field.swift, Manifest.swift, ManifestBuilder.swift, RuleRowView.swift, TrackImporter.swift (+2) | BuiltInSmartPresets.swift, FieldDefinitions.swift, LibraryStatsRepository+AudioQuality.swift, TagEditorSheet+InfoTabs.swift, TrackRepository+Provenance.swift (+2) | M001 |  |
| `title` | TEXT | n/a (common identifier, see notes) | n/a | M001 |  |
| `artist_id` | INTEGER | AlbumRepository+CollectionCards.swift, AlbumRepository.swift, ArtistEnrichmentService.swift, ArtistInfoSheet.swift, ArtistRepository.swift (+24) | AlbumDetailView.swift, AlbumsViewModel.swift, BatchCoverArtViewModel.swift, DuplicateReviewViewModel.swift, FieldDefinitions.swift (+14) | M001 |  |
| `album_artist_id` | INTEGER | AlbumRepository.swift, AlbumsViewModel.swift, ArtistRepository.swift, EditTransaction.swift, LibraryStatsRepository+Hygiene.swift (+4) | AlbumDetailView.swift, AlbumRepository+CollectionCards.swift, AlbumsGridView.swift, BatchCoverArtViewModel.swift, DeepDiveService.swift (+10) | M001 |  |
| `album_id` | INTEGER | A11yIdentifiers.swift, AlbumDetailView.swift, AlbumRepository.swift, AlbumsGridView.swift, AlbumsViewModel.swift (+37) | AlbumRepository+CollectionCards.swift, Artwork.swift, FieldDefinitions.swift, IdentifyTrackViewModel.swift, LibraryStatsRepository.swift (+12) | M001 |  |
| `track_number` | INTEGER | BackupRing.swift, EditTransaction.swift, Field.swift, FieldSelectionGrid.swift, FingerprintResult.swift (+25) | FieldDefinitions.swift, LibraryStatsRepository+Hygiene.swift, LibraryStatsRepository+ListeningBehaviour.swift, ScanCoordinator.swift, SmartPlaylist.swift (+7) | M001 |  |
| `disc_number` | INTEGER | BackupRing.swift, EditTransaction.swift, Field.swift, FieldSelectionGrid.swift, FingerprintResult.swift (+18) | FieldDefinitions.swift, LibraryStatsRepository+Hygiene.swift, LibraryStatsRepository+ListeningBehaviour.swift, ScanCoordinator.swift, TagEditorSheet.swift (+4) | M001 |  |
| `year` | INTEGER | n/a (common identifier, see notes) | n/a | M001 |  |
| `genre` | TEXT | n/a (common identifier, see notes) | n/a | M001 |  |
...
| `play_count` | INTEGER | Field.swift, ITunesLibraryReader.swift, LibraryStatsRepository+ListeningBehaviour.swift, LimitAndSortView.swift, PlayHistoryRecorder.swift (+11) | BuiltInSmartPresets.swift, FieldDefinitions.swift, LibraryListeningBehaviourPane.swift, SmartPlaylist.swift, SmartShuffle.swift (+2) | M001 |  |
...
| `content_hash` | TEXT | EpisodeStateRepository.swift, TrackImporter.swift, TrackRepository.swift | ContentHashService.swift, DownloadStore.swift, EpisodeDownloadManager.swift, FileServing.swift, ManifestBuilder.swift (+2) | ADR-065, #423 | Sync ETag from ContentHashService; kept on rescan while file_mtime is unchanged, cleared when the file changed (same rule as provenance). |
...
| `track_total` | INTEGER | BackupRing.swift, EditTransaction.swift, FieldSelectionGrid.swift, FingerprintResult.swift, FingerprintService.swift (+16) | AlbumRepository.swift, ScanCoordinator.swift, TagEditorSheet.swift, TagEditorViewModel+Conflict.swift, TrackInfoPanel.swift (+1) | M002, #404 | From TRACKTOTAL / TOTALTRACKS, or the N of an n/N TRACKNUMBER (ID3v2 TRCK, MP4 trkn) since #404. |
| `disc_total` | INTEGER | BackupRing.swift, EditTransaction.swift, FieldSelectionGrid.swift, FingerprintResult.swift, FingerprintService.swift (+14) | AlbumRepository.swift, ScanCoordinator.swift, TagEditorSheet.swift, TagEditorViewModel+Conflict.swift, TrackInfoPanel.swift (+1) | M002, #404 | Same sources as track_total, via DISCTOTAL / TOTALDISCS or n/N DISCNUMBER. |

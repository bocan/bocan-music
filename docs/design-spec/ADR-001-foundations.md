...
>
> Read `docs/design-spec/_standards.md` first. Every later ADR assumes this one is finished.
...
- Any persistence.
- Any dependency you don't need for ADR-001 itself.
...
1. **Xcode project**
   ...
   - Do **not** add `NSMusicFolderUsageDescription` yet — add in ADR-004 when the scanner lands.
...
6. **Brewfile**
   ...
   brew "swiftformat"
   brew "xcbeautify"
   brew "ffmpeg"        # needed in ADR-002, install now
   brew "create-dmg"    # needed in ADR-032, install now
   brew "gh"            # nice for the Actions workflow
   ```

7. **Makefile** — use exactly the targets in `spec.md` section ADR-001 step 3. Additionally:
...
- **Swift 6 and third-party packages**: most aren't strict-concurrency-clean yet. You don't depend on any here; in later ADRs expect to add per-target `.unsafeFlags(["-strict-concurrency=minimal"])` for offending packages with a TODO.
...
- **Hardened Runtime + FFmpeg** will bite in ADR-002 (disable library validation for FFmpeg dylib, or statically link). Don't do anything about it now — just be aware.

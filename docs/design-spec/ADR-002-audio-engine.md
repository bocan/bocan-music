...
- Queue, next/previous, gapless — ADR-006.
- Library, metadata, DB — ADR-003/3.
- EQ, ReplayGain — ADR-013.
...
15. **Clipping protection**: add an `AVAudioUnitEQ` in bypass mode now so the chain is ready for ADR-013. No gain changes yet.
...
- **Performance smoke**: decode a 10-minute FLAC in less than realtime × 0.1 (fast enough for pre-decode in ADR-006).
...
- **App Sandbox**: the file URL must come from a security-scoped bookmark (we don't have the bookmark machinery yet — that's ADR-004). For ADR-002, assume the caller has just opened the file via `NSOpenPanel`, which grants ephemeral access.
...
ADR-003 (Persistence) doesn't depend on this directly, but ADR-006 (Queue) will expect:

- `AudioEngine.load(_:)` can be called while another file is playing and cleanly replaces it.
- `AudioEngine` exposes the `AVAudioEngine` **indirectly** through an insertion point for later EQ (ADR-013) and taps (ADR-016). Do not expose the engine publicly; expose a protocol `AudioGraphInsertionPoint` that later ADRs conform filters to.
- Pre-decoding a next track is feasible: `Decoder` can be constructed and `read` can run without any engine involvement. (ADR-006 will exploit this.)

...
> Prerequisites: ADR-001–3 complete. A populated DB and working scanner.
>
> Read `docs/design-spec/_standards.md` first.

## Goal

Build the three-pane browser (sidebar | content | now-playing strip) so you can navigate and search your library, multi-select tracks, and trigger playback. **No queue or gapless logic yet** — clicking a track starts playback of that single track via the ADR-002 `AudioEngine`. Queue lands in ADR-006.

## Non-goals

- Queue management, next/previous — ADR-006.
- Playlists sidebar section beyond a placeholder — ADR-008.
- Smart playlists — ADR-009.
- Tag editor — ADR-010.
- Mini player — ADR-014.
- Visualizers — ADR-016.
...
3. **`Sidebar`** — sections:
   ...
   - **Playlists**: empty placeholder section (ADR-008 populates).
...
10. **Context menus** — in a reusable builder:
    - Play Now
    - Play Next *(stubbed no-op, enabled in ADR-006)*
    - Add to Queue *(stubbed, ADR-006)*
    - Add to Playlist ▸ *(stub, ADR-008)*
    - Love / Unlove
    - Rate ▸ (0–5 stars)
    - Show in Finder
    - Get Info *(stub, ADR-010)*
...
11. **Keyboard shortcuts** — centralised in `KeyBindings` and bound via `CommandMenu`/`.keyboardShortcut(...)` on views:
    ...
    - `⌘⇧N` new playlist (ADR-008 wires the action)
    - `⌘I` get info (ADR-010)
    - `⌘R` reveal in Finder
    - `⌘1…5` set rating
    - `⌘L` love
...
12. **Drag and drop**:
    ...
    - Dropping onto sidebar playlists is wired in ADR-008; use `DropDelegate` now but target an empty set of destinations.
...
public enum SidebarDestination: Hashable, Sendable, Codable {
    ...
    case playlist(Int64)         // ADR-008+
    case smartPlaylist(Int64)    // ADR-009+
    case search(String)
}
```
...
- **`⌘W`** must not terminate the app. Override the close behaviour to hide the window if mini-player mode is intended to survive (ADR-014 will finalise; stub the behaviour with a TODO).

## Handoff

ADR-006 (Queue) expects:

- `ContextMenus` has ready stubs for `Play Next` and `Add to Queue` that ADR-006 will wire.
- `NowPlayingViewModel` depends on a `Transport` protocol, not the concrete `AudioEngine` — ADR-006 will replace its engine with a `QueuePlayer` that conforms to `Transport`.
- Selection state is exposed as `Set<Track.ID>` so ADR-006 can build queues from selections.

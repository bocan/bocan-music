# ADR-006 — Playback Queue & Gapless Playback

> Prerequisites: ADR-001–4 complete. UI can trigger single-file playback.
>
...
- Playlists as persistent entities — ADR-008.
- Smart playlists — ADR-009.
- EQ / ReplayGain on output — ADR-013 (but the engine must already have the insertion point from ADR-002).
- Crossfade between tracks — ADR-013.
...
5. **`QueuePlayer`** — conforms to `Transport` from ADR-002 so the UI can replace the engine. Responsibilities:
...
6. **`GaplessScheduler`** — the heart of this ADR.
   ...
   - `GaplessScheduler` lives inside `QueuePlayer` and uses an insertion point on `AudioEngine` exposed in ADR-002 (`AudioGraphInsertionPoint`).
...
ADR-008 (Manual Playlists) expects:

- `QueuePlayer.playAlbum` / `playArtist` are implemented generically in terms of "play a sequence of Track.IDs in this order"; ADR-008 will call the same entry point with a playlist's track IDs.
- Context menus have "Add to Playlist ▸" as a stub that ADR-008 populates.
- `QueueView` understands the `QueueItem` model cleanly; ADR-008 will reuse row rendering in the playlist view.

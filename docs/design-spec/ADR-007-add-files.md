# ADR-007 — Add Media to Library

> Prerequisites: ADR-006 (Queue & Gapless) complete.
>
> This is an interstitial ADR: the Library scanning engine was built in ADR-004,
> but its UI surface was deferred. Without a way to add music, the app is an empty
> shell. ADR-007 fills that gap before any further features are built.
...
└── TrackInspectorPanel.swift  # Read-only ⌘I info panel; ADR-010 replaces it with the full editor
```
...
- **Error state**: "⚠ N errors during scan" — tap to see list (future ADR).
...
ADR-010 converts this into a writable editor using the same `⌘I` shortcut.
...
**Schema** — add a column to `albums` via a new migration (numbered to slot before ADR-009’s smart-playlist migration):
...
- Removing tracks that were deleted from disk (ADR-005 incremental sync).
- Editing tags (ADR-010).
- Watching folders for live changes (FSWatcher wired in ADR-005).
- Showing per-file errors in detail (noted in banner, expanded in ADR-010).
- Editing tags in the inspector (ADR-010 full editor).

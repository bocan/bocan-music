# ADR-008 — Manual Playlists

> Prerequisites: ADR-001–5 complete. Queue works. UI context-menu stubs exist.
>
...
- Smart (rule-based) playlists — ADR-009.
- Import/export (M3U, etc.) — ADR-030.
- Collaborative/shared playlists — out of scope.
- Playlist sync via iCloud — not in v1 (DB backup in ADR-003 is the nearest thing).
...
1. **Confirm schema** from ADR-003 has everything we need:
...
11. **Persistence of UI state** — which folders are expanded, which playlist was last selected. Encoded in the `ui.state.v1` JSON already established in ADR-005.
...
- **`playlist_tracks` ON DELETE CASCADE** — if missing from M001, add a migration to recreate the FK (SQLite quirk: altering a FK means renaming the table). Much easier to get right the first time in ADR-003.
...
- **Sidebar drag acceptance**: only accept drops of known `Transferable` types. Reject random strings and files (those go to the import flow in ADR-030).
...
ADR-009 (Smart Playlists) expects:
...
- `PlaylistService.create` has a sibling `createSmart(name:criteria:parentID:)` added in ADR-009 with the same folder/tree rules.

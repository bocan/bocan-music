# ADR-010 — Metadata Editor & Cover Art Fetching

> Prerequisites: ADR-001–7 complete. TagLib bridge (ADR-004) includes writing support.
>
...
- Acoustic fingerprinting / AcoustID — we'll build this in ADR-011.
- Auto-tagging entire libraries from nothing — out of scope.
- Lyric fetching — ADR-015 (note: keep architecture compatible).
...
7. **User-edited flag** — after any successful edit, set `tracks.user_edited = 1` so ADR-004's scanner respects user priority on next scan.

8. **Conflict resolution** — when the scanner's `ConflictResolver` flagged a track (ADR-004), the editor sheet shows a banner: "This file was changed on disk after your last edit." with buttons "Keep my edits", "Take disk version", "Show diff". The diff is a side-by-side field comparison.
...
16. **Tools menu**:
    ...
    - "Recompute ReplayGain" (forwards to ADR-013 if available, otherwise disabled) — hook present in UI, logic may be stubbed here.
...
- Editing `replaygain_track_gain` / `replaygain_album_gain` fields exists through the patch type (advanced section). ADR-013 will populate these when computing ReplayGain.

# ADR-011 — AcoustID Fingerprinting & MusicBrainz Auto-Tagging

> Prerequisites: ADR-010 (Metadata Editor) complete. ADR-004 TagLib bridge includes writing support.
>
> Read `docs/design-spec/_standards.md` first.

## Goal

Identify tracks whose metadata is missing or incorrect by computing an acoustic fingerprint (via Chromaprint / fpcalc), submitting it to the AcoustID lookup service, resolving the result against MusicBrainz, and offering the user confirmed tag data to apply through the ADR-010 tag writer.
...
Add via a new migration (M005, after ADR-013's M004):
...
6. **`IdentifyTrackSheet`** — modal sheet, opens via:
   ...
   - **No match**: "No match found. Try editing tags manually." with a link to open the ADR-010 tag editor.
...
7. **`CandidatePickerView`** — displays candidates as a list:
   ...
   - "Apply" button (primary) → calls `MetadataEditService.apply(_:to:)` via ADR-010's tag writer. Shows a brief success toast.
...
- [ ] "Apply" writes metadata to the file via ADR-010's `MetadataEditService`.
...
- **MusicBrainz `User-Agent`**: the policy mandates a valid contact URL. Use `https://github.com/bocan/bocan-music`. A missing or generic `User-Agent` will result in 403 responses.
...
ADR-013 (EQ/Effects) and ADR-029 (Last.fm scrobbling) expect:

# ADR-013 — Multi-Band EQ, ReplayGain & Audio Effects

> Prerequisites: ADR-001–8 complete. `AudioEngine` exposes an `AudioGraphInsertionPoint` (ADR-002).
>
...
Modules/Playback/Sources/Playback/Gapless/   # Extended from ADR-006
...
16. **`CrossfadeScheduler`** — extends the `GaplessScheduler` handoff (ADR-006) with a volume-ramp transition when crossfade is enabled. Lives in `Modules/Playback`.
    - Setting: `playback.crossfade.durationSeconds` (0.0 = off, 1.0–10.0 s). Stored in `settings` table (no schema migration needed).
    - **“Keep gapless within albums”** option (`playback.crossfade.albumGapless`, default `true`): tracks from the same album use the existing ADR-006 gapless path (sample-accurate handoff); crossfade only applies at album boundaries.
    ...
    - When `durationSeconds = 0`: code path is identical to ADR-006’s hard handoff — no regression.
...
- At `crossfadeSeconds = 0`, output is bit-identical to ADR-006 gapless fixture test.
...
- [ ] `crossfadeSeconds = 0` is bit-identical to ADR-006 gapless behaviour.
...
- **Sample rate**: filter coefficients depend on sample rate. On device change (ADR-002 already reconfigures the engine), recompute all filter coefficients.
...
ADR-014 (Mini Player + Polish) expects:

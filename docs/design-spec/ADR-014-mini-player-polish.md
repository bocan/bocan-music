# ADR-014 — Mini Player, Window Modes, Polish

> Prerequisites: ADR-001–9 complete.
>
...
```
Modules/Playback/Sources/Playback/    # Extended from ADR-006
...
├── DSPSettingsView.swift            # Wraps ADR-013 views
...
ADR-015 (Lyrics) expects:

- A reusable overlay/sheet surface exists so a lyrics pane can appear without disrupting transport.
- `MiniPlayerView` has a lyrics toggle stub (disabled until ADR-015 ships).
- Accessibility settings from this ADR apply to the lyrics view (reduce motion disables auto-scroll).

...
> Prerequisites: ADR-001–10 complete. `LRCParser` exists in `Metadata` (ADR-004). `lyrics` table exists.
>
...
6. **`LyricsEditorSheet`**:
   ...
   - Save options: save to DB (always); additionally embed in file (requires ADR-010 write path and settings opt-in).
...
- No lyrics view grabs the audio tap; ADR-016 installs the tap on the engine without contention.

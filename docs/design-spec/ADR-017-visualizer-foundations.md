# ADR-017: Visualizer Foundations (Analysis v2 + Dynamic Palettes)

> Prerequisites: ADR-016 complete. `AudioTap` + `FFTAnalyzer` exist and feed
...
Shared groundwork for the four new visualizer modes (ADR-018 to ADR-021):
...
- No new visualizer modes here (those are ADR-018 to ADR-021).
...
ADR-018 (Halo), ADR-019 (Cascade), ADR-020 (Starfield), ADR-021 (Nebula) expect:

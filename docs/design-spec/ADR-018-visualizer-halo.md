# ADR-018: Visualizer Mode "Halo" (Radial Spectrum + Beat Ripples)

> Prerequisites: ADR-017 complete (Analysis v2, `PaletteResolver`,
> `render(... time:)`).
>
...
- `reduceMotion`: rotation off, ripples off, breathing off; the ring still
  ...
  3 times slower). This matches the calm-still-life convention from ADR-016.
...
ADR-019 (Cascade) is independent. The ripple-pool pattern (fixed-size
struct pool keyed on birth time) is reused by Starfield (ADR-020) for its warp

# ADR-019: Visualizer Mode "Cascade" (Scrolling Spectrogram Waterfall)
...
- **Memory**: bitmap and LUT allocations happen in `init` only; long-run
  render of 10k frames shows no growth (ties into the ADR-016 long-run gate).
...
- **Analysis identity.** `Analysis` is a value type with no counter; detect
  ...
  comparing 32 floats. (Add `frameIndex: UInt64` to `Analysis` in this ADR;
...
ADR-020 (Starfield) reuses `Analysis.frameIndex` for per-frame triggers.
Nebula (ADR-021) reuses the `rampStops` LUT idea as a Metal texture.

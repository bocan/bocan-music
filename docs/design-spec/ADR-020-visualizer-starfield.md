# ADR-020: Visualizer Mode "Starfield" (Frequency-Coloured Warp Field)

> **Renderer superseded.** Implement this mode via
> `ADR-027-visualizer-metal-starfield.md` (Metal), not as a Canvas
...
> `render(... time:)`). ADR-019 adds `Analysis.frameIndex` (used here).
>
...
ADR-021 (Nebula) reuses: the onset-envelope pattern (attack/decay uniform),

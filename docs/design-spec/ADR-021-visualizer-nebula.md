# ADR-021: Visualizer Mode "Nebula" (Metal Gas Clouds + Moving Wisps)

> **Plumbing superseded.** Implement this mode via
> `ADR-028-visualizer-metal-nebula.md`, which lands it on the shared
> Metal foundations from ADR-022. The visual design, audio mapping, test
> plan, and gotchas below remain binding; the bespoke NebulaMetalView and
> NebulaRenderer plumbing does not.
>
> Prerequisites: ADR-017 complete (Analysis v2, `PaletteResolver.rampStops`,
> `render(... time:)`). ADR-018 to ADR-020 are independent but land first;
> this is the most ambitious mode and finally delivers the Metal mode that
> ADR-016 specified as "Fluid" and never shipped.
>
...
- No shader plugin system (still ADR-016's non-goal).
...
- `reduceMotion`: the mode is unavailable, exactly as ADR-016 specified for
...
- First-frame warm-up (ADR-016 gotcha): the view stays at alpha 0 over the
...
ADR-016). Verify SwiftPM compiles `Nebula.metal` into the module's
...
CPU side (the coverage target; the GPU pass is exempt, as ADR-016 already
...
- **`MTKView` lifecycle in SwiftUI.** Dismantle must stop the draw loop
  ...
  pane; verify with the fullscreen open/close test from ADR-016.
...
conventions when these ship), and revisiting ADR-016's unchecked long-run

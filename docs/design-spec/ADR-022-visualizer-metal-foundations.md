# ADR-022: Metal Visualizer Foundations (MetalVisualizer Protocol + Shared Infrastructure)

> Prerequisites: ADR-017 (Analysis v2, `PaletteResolver`), ADR-018 (Halo),
> ADR-019 (Cascade, `Analysis.frameIndex`) complete and on `main`.
>
...
> 60 fps but with visible latency and CPU cost. This ADR builds the shared
> Metal rendering infrastructure; ADR-023 to ADR-028 move individual modes
> onto it. **This ADR has zero user-visible changes.** No new modes, no
> changed pixels, no new strings.

## Goal

A `MetalVisualizer` protocol parallel to the existing `Visualizer` protocol,
plus everything a converted mode needs so the per-mode ADRs stay small:
...
- No mode is converted here (ADR-023 to ADR-026 do that, one mode per ADR).
- No Canvas renderer is deleted, ever, in this ADR series. Canvas renderers
  remain the fallback when Metal is unavailable (VMs, exotic CI) and the
  reference implementation for visual parity.
- No new visualizer modes (Starfield is ADR-027, Nebula is ADR-028).
...
└── (empty in this ADR; per-mode .metal files arrive in ADR-023+)
...
public protocol MetalVisualizer: AnyObject {
    ...
    /// Default 1.0 via protocol extension. Nebula (ADR-028) overrides this for
...
enum MetalVisualizerFactory {
    /// True when `mode` has a Metal renderer. ADR-022 returns false for every
    /// mode; each conversion ADR adds its arm.
    ...
    /// success. conversion ADRs call this from `make`.
...
- **Warm-up fade**: the view starts at `alphaValue = 0` over the host's black
  ...
  reduce motion, jump straight to 1). No spinner. This is the ADR-016 gotcha
...
- **Teardown**: `dismantleNSView` sets `isPaused = true` and `delegate = nil`,
  or the GPU keeps drawing for a closed pane (ADR-016 fullscreen open/close
...
type. Canvas `Cascade` is refactored to use it **in this ADR**, with the
...
The attack/decay envelope both Starfield (ADR-027) and Nebula (ADR-028) need, and
...
- **Vertex pulling, not vertex descriptors.** Vertex functions index typed
  ...
  No `MTLVertexDescriptor` anywhere in this ADR series; it removes an entire
...
- **Coverage.** Keep `encode(into:)` mechanical (bind, draw, repeat); every
  ...
  exemption ADR-016 documented for Metal render paths.

## Handoff

ADR-023 to ADR-028 each convert or add one mode. They expect: the protocol
...
`visualizer.forceCanvas` escape hatch. Recommended order: ADR-023 Oscilloscope
(simplest, proves the ribbon + uniform path), ADR-024 Cascade (proves the
texture + LUT path), ADR-025 Spectrum Bars (proves instancing), ADR-026 Halo,
ADR-027 Starfield, ADR-028 Nebula.

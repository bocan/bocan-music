# ADR-023: Metal Oscilloscope (First Mode Conversion)

> Prerequisites: ADR-022 complete (MetalVisualizer protocol, factory,
...
Convert the Oscilloscope to Metal as the proving run for the ADR-022
...
1. **Coordinate spaces, decided once for the whole ADR series.** `update`
   ...
   Document both in code; every later conversion ADR reuses this recipe
   verbatim.
...
5. `encode`: set pipeline, bind the FrameRing buffer, draw both triangle
   ...
   `ring.release(when:)` in `didEncode(commandBuffer:)` (the ADR-022 protocol
...
- **This ADR is the template.** Patterns established here (pixelsPerPoint
  seam, didEncode/FrameRing pairing, snapshot-suite shape) are copied by
  ADR-024 to ADR-028; sloppiness here multiplies by five.
...
ADR-024 (Cascade) and every later conversion reuse the patterns established

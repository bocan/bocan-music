# ADR-024: Metal Cascade (Spectrogram on a GPU Texture)

> Prerequisites: ADR-022 (foundations, including the `PaletteRampLUT`
> extraction) and ADR-023 (Oscilloscope conversion; establishes the
> pixelsPerPoint seam, FrameRing pairing, and snapshot patterns) complete.
>
> Read `docs/design-spec/_standards.md` first, then
> `ADR-019-visualizer-cascade.md` for the original design rationale.
...
- LUT: `PaletteRampLUT` (shared since ADR-022), colours applied at write time;
...
- **Marker column orientation**: write one column with band 15 = 1.0, render
  ...
  *bottom* half when the marker band is bass-side (band 15 of 32 sits mid
  ...
  is the upside-down-spectrogram guard from ADR-019, re-asserted for the new
  pipeline.
...
ADR-025 (Spectrum Bars) needs nothing new from this ADR. ADR-028 (Nebula) reuses
the `PaletteRampLUT.makeTexture`/`upload` path this ADR exercises first;

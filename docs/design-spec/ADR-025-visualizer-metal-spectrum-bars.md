# ADR-025: Metal Spectrum Bars (Instanced Quads)

> Prerequisites: ADR-022 and ADR-023 complete. ADR-024 is recommended first but
> not required.
>
...
plus 32 peak-marker instances, one draw call each. This ADR proves the
instancing pattern that Halo's ripples (ADR-026) and Starfield's 500 stars
(ADR-027) depend on.
...
ADR-026 (Halo) reuses the instanced-SDF pattern for ripples and the centre
glow; ADR-027 (Starfield) scales it to 500 instances.

# ADR-026: Metal Halo (CPU Geometry, GPU Rasterisation)

> Prerequisites: ADR-022, ADR-023 (PolylineRibbon patterns), ADR-025
> (instanced-SDF pattern) complete.
>
> Read `docs/design-spec/_standards.md` first, then
> `ADR-018-visualizer-halo.md` for the original design.
...
6. **Ripples + glow**: instanced SDF quads (the ADR-025 pattern): up to 6 ring
...
ADR-027 (Starfield) reuses the instanced-SDF pipeline shape at 500 instances

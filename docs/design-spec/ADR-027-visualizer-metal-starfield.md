# ADR-027: Starfield on Metal (New Mode, Supersedes ADR-020's Renderer)

> Prerequisites: ADR-022 (foundations, `OnsetEnvelope`), ADR-025
> (instanced-SDF pattern) complete.
>
> **This ADR supersedes the rendering technology of
> `ADR-020-visualizer-starfield.md`. Do not implement the Canvas Starfield
> first.** Everything in ADR-020 about visual design, motion, accessibility, and
...
>
> Read `docs/design-spec/_standards.md` and `ADR-020-visualizer-starfield.md`
> first.

## Goal

Implement the Starfield mode (frequency-coloured warp field, 500 stars, warp
kicks on onsets) directly as a `MetalVisualizer`. Unlike ADR-023 to ADR-026 there
is no Canvas twin: Starfield is the first Metal-only mode, which makes it the
ADR that introduces the "mode requires Metal" machinery that Nebula (ADR-028)
also needs.

## Non-goals

- Everything ADR-020 lists (no 3D projection, no particle interaction), plus:
...
## Mode availability machinery (new in this ADR, reused by ADR-028)
...
public extension VisualizerMode {
    ...
    var requiresMetal: Bool { self == .starfield }   // ADR-028 adds .nebula
}
```
...
- `VisualizerSettingsView` mode picker: options where
  ...
  - `"Requires Metal graphics support, which this Mac does not provide."`
    (shared tooltip key; ADR-028 reuses it)

## Behavioural contract (from ADR-020, restated where load-bearing)
...
- Warp kick: `warpBoost = 2.0 * onsetEnvelope.value` with
  `OnsetEnvelope(tau: 0.4)` (the ADR-022 helper; onset edge-detection via
...
seam). All ADR-020 motion/respawn/twinkle/warp rules live here. This split is
...
ADR-025/ADR-026 shared instanced-SDF pipeline with a shape flag if ADR-026 landed
one.
...
- All of ADR-020's test plan items that concern simulation: determinism under a
...
- [ ] All simulation behaviour matches ADR-020's contract; its applicable test
      plan items pass.
...
- ADR-020's pool/streak/frame-rate-independence/onset-edge gotchas all still
  apply; read them.
...
ADR-028 (Nebula) extends `requiresMetal`, reuses the shared tooltip key, the

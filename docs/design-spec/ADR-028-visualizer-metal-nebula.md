# ADR-028: Nebula on the Metal Foundations (Delta Over ADR-021)

> Prerequisites: ADR-022 (foundations), ADR-027 (Starfield: `requiresMetal`
> machinery, shared tooltip key, `OnsetEnvelope` in anger) complete.
>
> **This ADR is a delta over `ADR-021-visualizer-nebula.md`.** ADR-021
...
> wiring, its own metallib strategy). All of ADR-021's **visual design, audio
...
>
> Read `docs/design-spec/_standards.md`, `ADR-021-visualizer-nebula.md`,
> and `ADR-022-visualizer-metal-foundations.md` first.

## Goal

Implement Nebula (domain-warped fBm gas, four wisps, onset pressure waves) as
a `MetalVisualizer` on the ADR-022 foundations, completing the six-mode set.

## What carries over from ADR-021 unchanged
...
| ADR-021 said | Now |
|---|---|
| `NebulaMetalView.swift` (bespoke NSViewRepresentable) | Deleted from scope. `MetalVisualizerView` (ADR-022) hosts it like every other mode. |
| `NebulaRenderer.swift` (bespoke MTKViewDelegate, pacing, watchdog) | `MetalNebula: MetalVisualizer`. Pacing, watchdog ticks, warm-up fade, and teardown all come from the ADR-022 host. |
| "Verify SwiftPM compiles Nebula.metal into default.metallib; fall back to runtime compilation if the toolchain disappoints" | Decided in ADR-022: runtime compilation via `MetalShaderLibrary`, always. `Nebula.metal` lives in `Resources/Shaders/` like the others. |
| Own onset envelope implementation | `OnsetEnvelope(tau: 0.3)` from ADR-022. |
| Own LUT texture code | `PaletteRampLUT.makeTexture` / `upload` from ADR-022 (exercised by ADR-024). The centroid tint stays a shader-side uniform offset, not a LUT rebuild. |
| `VisualizerHost` gains a special `.nebula` Metal branch | Already generic: factory arm + `requiresMetal` includes `.nebula` (extend the ADR-027 property). |
| Own first-frame warm-up fade | ADR-022 host behaviour. |
| Adaptive render scale implemented ad hoc | The `renderScale` protocol property (ADR-022), overridden here: default 0.6, drop to 0.4 when the rolling CPU-side frame budget is exceeded, recover with hysteresis exactly per ADR-021's test plan. The ADR-022 host re-reads `renderScale` every frame, so no extra wiring. |
...
shared key from ADR-027.

## Availability matrix (implement exactly this)

| Condition | Picker option | Host renders |
|---|---|---|
| Metal absent | disabled, ADR-027 tooltip | SpectrumBars fallback |
...
2. `NebulaUniforms` packing: flowTime integrator, `OnsetEnvelope`, wisp
   ...
   per ADR-021's CPU test plan. Commit.
...
ADR-021's list still applies, minus the SwiftPM metallib item (decided), plus:
...
ADR-021's test plan verbatim, with these substitutions:

- "Watchdog wiring" is already covered by ADR-022's host tests; keep only a
  source-convention assertion that Nebula goes through the standard factory.
- The onset-envelope contract tests live in ADR-022; here test only the tau
...
ADR-021's list, re-read it, plus:
...
- **renderScale interacts with aspect correction**: the fragment shader's
  ...
  at 0.4x.
- **The ADR-022 host re-reads `renderScale` per frame**; make the property a
...
pages for the visualizer suite, and the ADR-016 long-run memory soak across

# ADR-029 — Last.fm & ListenBrainz Scrobbling

> Prerequisites: ADR-001–12 complete. `play_history` and `scrobble_queue` tables exist (ADR-003).
>
...
4. **`ScrobbleRules`**:
   ...
   - Skips (next pressed before threshold) → no scrobble; `skip_count` still updated (already handled in ADR-006's recorder).
   - **One scrobble per play event**; replays count as new events.

5. **Integration with `PlayHistoryRecorder` (ADR-006)**:
...
- **Scrobble rule "playback time"** excludes pauses. The recorder from ADR-006 tracks that already; ensure `PlayEvent.playedAt` is the *original* track start time, not the resumed timestamp (Last.fm expects when the play *began*).
...
- **Multiple workspaces / instances**: if the user runs two copies of Bòcan simultaneously (don't), scrobble duplicates can appear. Enforce a single-instance policy in ADR-032 or accept the trade-off.

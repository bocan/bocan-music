# ADR-030 — Playlist Import / Export

> Prerequisites: ADR-001–13 complete. Playlist CRUD (ADR-008) and smart playlists (ADR-009) stable.
>
...
13. **Export path for the library** — "Bòcan Backup" flow: writes all playlists + a CSV of plays and ratings. Not a full backup (DB backup lives in ADR-003), but a readable snapshot.

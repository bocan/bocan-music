...
> Prerequisites: ADR-001–16 complete. `QueuePlayer` exposes the full `Transport`
...
- **Section name in the sidebar is "Sources"**, not "Subsonic / Navidrome".
  Rationale: Subsonic is just one source family. A future ADR can add Plex,
...
- **"Recents" stays local-only for now.** Mixing remote and local Recently
  ...
  bigger problem than this ADR should solve. ADR-036 (a future ADR) can
  unify recents. This ADR only sources Recently Added from the server inside
...
- **Star / rating / scrobble are write-through.** Starring a Subsonic track in
  ...
  call `scrobble`. Local scrobbling (ADR-029) still runs in parallel, the
...
- Becoming a Subsonic *server*. ADR-034 covers the remote-control server;
  this ADR is purely a client.
...
`.subsonic(server, …)`, the existing scrobble pipeline (ADR-029) calls
...
15. **Scrobble write-through** at end-of-track. Hook into the existing
    ADR-029 scrobble dispatch, not the engine.
...
- **Network changes (`NWPathMonitor`).** When the user switches Wi-Fi, all
  ...
  on path changes.
- **Multi-server playlists**. A manual Bòcan playlist (ADR-008) can mix
...
The next ADR (ADR-036, future) is expected to:

# ADR-036: In-App Log Console

> Prerequisites: ADR-001 to ADR-035 complete. `AppLogger` (`Observability`) is the
...
This ADR delivers exactly that, with two deliberate decisions called out up
...
- A remote / networked log sink, log shipping, or crash upload. MetricKit
  crash reporting (ADR-032 / `DiagnosticsSettingsView`) is unchanged and
  separate.
- Structured field inspection (expandable key/value trees). We store the same
  flat, formatted string that goes to `os.Logger`. A future ADR can enrich
...
A later ADR could, on top of this:

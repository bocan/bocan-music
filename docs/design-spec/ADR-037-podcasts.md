# ADR-037: Podcasts - Overview and Cross-ADR Contract

> Prerequisites: ADR-001 to ADR-036 complete. The module DAG, `QueuePlayer` /
...
|------|-------|-------------------|
| `ADR-037-podcasts.md` | This contract (read first) | - |
| `ADR-038-podcast-persistence.md` | Schema, records, repositories | Persistence |
| `ADR-039-podcast-feeds.md` | `Podcasts` module scaffold, feed fetch + RSS/Atom parse | Podcasts |
| `ADR-040-podcast-search.md` | Podcast Index + iTunes dual search, dedupe/merge | Podcasts |
| `ADR-041-podcast-subscriptions.md` | `PodcastService` facade: subscribe/refresh/state, artwork cache | Podcasts |
| `ADR-042-podcast-playback.md` | `PlayableSource.podcast`, resolver seam, resume + position write-back | Playback, App |
| `ADR-043-podcast-downloads.md` | Episode downloads + offline (optional, post-MVP) | Podcasts, App |
| `ADR-044-ui-podcasts-home.md` | Sidebar item, subscribed grid, add bar, UI seam protocols | UI, App |
| `ADR-045-ui-search-detail.md` | Search results list (source badges), detail + Subscribe | UI |
| `ADR-046-ui-episodes.md` | Episode table (date/duration/progress), show notes, context menu | UI |
| `ADR-047-nowplaying-polish.md` | Now Playing podcast mode, speed/skip, settings, docs, l10n sweep | UI, App |

The MVP is ADR-038, ADR-039, ADR-040, ADR-041, ADR-042, ADR-044, ADR-045, ADR-046. ADR-043
(downloads) and the speed/settings/polish in ADR-047 are enhancements that make
...
- **The local "Podcasts" library is distinct from the existing Subsonic server
  ...
  ADR is the local, feed-based library.
- **Downloads are a first-class but optional enhancement.** Streaming (play the
  ...
  MVP. Downloading for offline + bulletproof seeking is ADR-043.
...
- Chapters UI and transcript display. We persist `chapters_url` /
  `transcript_url` so a future ADR can add them, but rendering them is out of
  scope.
- Re-implementing an RSS parser by hand. Wrap FeedKit (see ADR-039).
...
dependencies" table) as part of ADR-039:
...
set of protocols declared in `UI` (see ADR-044) and implemented in `App`.

## Data model (the spine)

Three tables, added in one migration (ADR-038). The next free migration
...
## Shared value types (referenced across ADRs)

These are the boundary types. Their canonical home is noted; other ADRs
consume them as named here.

```swift
// Persistence module (ADR-038). The read model the UI renders: an episode
...
```swift
// Podcasts module (ADR-039). The normalized output of feed parsing, source
...
```swift
// Podcasts module (ADR-040). One deduplicated, merged search hit.
...
```swift
// Playback module (ADR-042). The App-injected seam, mirroring
...
Used in two places that must agree: the search dedupe key (ADR-040) and the
`podcasts.feed_url` uniqueness + subscribe lookup (ADR-041). Implement once in
...
1. User selects **Podcasts** in the sidebar (`SidebarDestination.podcasts`).
   ...
   subscribed shows below (ADR-044).
2. User types in the Add bar. `PodcastSearchService` fans out to Podcast Index
   ...
   The results list shows artwork, title, author, and a source badge (ADR-045). If the text is a feed URL, an "Add this feed" affordance appears.
3. User clicks a result. The detail view fetches and parses the feed
   ...
   episodes, and a **Subscribe** button (ADR-045).
4. Subscribe calls `PodcastService.subscribe(feedURL:)`: parse, upsert the
   ...
   (ADR-041). The grid now shows the new show.
5. User clicks the show (`SidebarDestination.podcastShow(id)`). The episode
   ...
   indicator derived from `state` (ADR-046).
6. User plays an episode. The UI builds a podcast `QueueItem`
   ...
   `audioURL` and `resumePosition`, seeks to the resume point, and plays (ADR-042).
...
8. Now Playing shows the episode in podcast mode: show art, episode title, show
   name; skip-back/skip-forward and a speed control (ADR-047).

## Build, test, and wiring checklist (applies to every ADR)
...
- Add a `make test-podcasts` target to the root `Makefile` mirroring the other
  ...
  `coverage-all` machinery. Do this in ADR-039 when the module first exists.
...
## Localization note (carry into every UI ADR)

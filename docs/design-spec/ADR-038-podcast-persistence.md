# ADR-038: Podcasts - Persistence (schema, records, repositories)

> Depends on: `ADR-037-podcasts.md` (read it first; the table shapes and the
> "state is a separate table" rationale live there). No other ADR-037 file is a
> prerequisite. This slice touches only the **Persistence** module.
>
> Provides, for later ADRs: the `M023_Podcasts` migration; the `Podcast`,
...
ADR-037 builds on. Follow the existing Persistence conventions exactly
...
- No FTS for podcasts in this slice. The local library is small; the UI filters
  ...
  predicates (ADR-046). A full-text index can be a later add if it is ever
  warranted.
- No business logic (subscribe/refresh/dedupe). That is the `Podcasts` module
  (ADR-039, ADR-041). This slice is pure storage.
...
exact SQL from `ADR-037-podcasts.md` ("Data model"). Then register it in
...
Exactly as defined in `ADR-037-podcasts.md`. It is not a GRDB record; it is the
...
ADR-039 (the `Podcasts` module) consumes nothing from this slice directly
(it returns value types), but ADR-041 (`PodcastService`) injects these three
repositories. ADR-042 (playback) reads/writes state through
`EpisodeStateRepository` via the App-layer resolver. ADR-044 and ADR-046 render

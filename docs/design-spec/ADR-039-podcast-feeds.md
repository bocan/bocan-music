# ADR-039: Podcasts - Module scaffold, feed fetch, RSS + Atom parsing

> Depends on: `ADR-037-podcasts.md`. This slice **creates the `Podcasts`
...
>
> Provides, for later ADRs: the module + its `Package.swift` + `project.yml`
...
- No subscribe/refresh orchestration (ADR-041 owns that; it calls this).
- No search clients (ADR-040, same module, separate slice).
- No artwork download (ADR-041).
...
> Persistence is a dependency now (ADR-040 and ADR-041 in this same module need
...
1. `project.yml`: add `Podcasts: { path: Modules/Podcasts }` to `packages:` and
   ...
   with the others). The UI dependency on Podcasts is added in ADR-044.
...
One `Error, Sendable` enum for the whole module (used by this slice and by ADR-040,
ADR-041). Cases carry context:
...
public enum PodcastsError: Error, Sendable, CustomStringConvertible {
    ...
    case searchUnavailable(source: String, reason: String)   // used by ADR-040
    case notFound(feedURL: URL)             // used by ADR-041
    case keychain(OSStatus, String)         // reserved if ADR-040 stores PI creds in Keychain
...
The single source of truth for canonicalization, used here, in ADR-040 (dedupe), and
in ADR-041 (uniqueness). Implement the rules from `ADR-037-podcasts.md` ("Feed URL
canonicalization"):
...
- **Redirects.** A feed often 301s to a new host (hosting migrations). Capture
  `finalURL` now; persisting the moved URL is a ADR-041 nicety, but losing the
...
ADR-040 adds the search clients to this module. ADR-041's `PodcastService`
calls `FeedFetcher` + `FeedParser` to subscribe and refresh, mapping `ParsedFeed`
to the `Podcast` / `PodcastEpisode` records from ADR-038.

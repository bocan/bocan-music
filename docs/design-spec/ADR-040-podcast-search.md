# ADR-040: Podcasts - Dual-index search (Podcast Index + iTunes), dedupe/merge

> Depends on: `ADR-037-podcasts.md`, `ADR-039-podcast-feeds.md` (the `Podcasts`
...
detail/subscribe flow are ADR-045 / ADR-041.

## Non-goals

- No UI. The result list, badges, and detail view are ADR-045.
- No subscription writes. Subscribe is ADR-041.
- Episode-level search inside the index APIs is out of scope; search is
  show-level discovery. (We fetch episodes by parsing the feed in ADR-041/ADR-045.)
...
1. Bucket every result by `FeedURL.canonicalKey(result.feedURL)` (from ADR-039).
...
(ADR-045 calls `FeedFetcher` + `FeedParser`), not from the index APIs, so this
...
- **iTunes feed URLs are sometimes stale.** Apple's index lags; Podcast Index is
  ...
  and why subscribe (ADR-041) always re-parses the live feed rather than trusting
  index metadata.
- **Canonical-key dedupe must use the shared `FeedURL.canonicalKey`** from ADR-039,
...
- **Rate limits.** Podcast Index and iTunes both throttle. Debounce search input
  in the UI (ADR-045, ~300 ms) and cancel the in-flight `search` task when the
...
ADR-045 calls `PodcastSearchService.search` for the results list and
`detail(for:)` + `FeedParser` for the detail view. ADR-041 reuses

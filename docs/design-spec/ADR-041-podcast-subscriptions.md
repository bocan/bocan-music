# ADR-041: Podcasts - PodcastService facade (subscribe, refresh, state, artwork)

> Depends on: `ADR-037-podcasts.md`, `ADR-038-podcast-persistence.md` (records +
> repositories), `ADR-039-podcast-feeds.md` (`FeedFetcher`, `FeedParser`,
> `FeedURL`, `PodcastsError`). ADR-040 is optional here (only the subscribe
> path can opportunistically carry index IDs).
>
> Provides: `PodcastService` - the single public facade the App layer wires into
> both the UI data-source seams (ADR-044) and the playback resolver seam (ADR-042);
...
- No UI (ADR-044 to ADR-047).
- No `PlayableSource` / `QueuePlayer` changes (ADR-042). This slice only
  exposes the state read/write methods the resolver will call.
- No downloads (ADR-043). `PodcastArtworkCache` here is only for cover art.
...
public actor PodcastService {
    ...
    // MARK: Reads (feed the UI data-source seam in ADR-044)
    ...
    // MARK: Playback-state bridge (the resolver in ADR-042 calls these)
...
implementation (ADR-042) forwards to. They translate `feedURL + guid` into
`(podcastID, guid)`:
...
- **Effective-completion threshold (15 s) is shared** with the player (ADR-042) and
  the UI progress indicator (ADR-046). Define it once as a constant in the module
...
ADR-042's App-layer `PodcastEpisodeResolving` implementation forwards directly
to `audioURL` / `resumePosition` / `saveProgress` / `markPlayed`. ADR-044's
App-layer UI data source forwards to `subscribedPodcasts` / `episodes` / the
observations and to `subscribe` / `unsubscribe` / `refresh`. ADR-045 calls

# ADR-042: Podcasts - Playback integration (PlayableSource.podcast, resume, position write-back)

> Depends on: `ADR-037-podcasts.md`, `ADR-038-podcast-persistence.md`,
> `ADR-041-podcast-subscriptions.md` (the `PodcastService` state bridge). Touches
...
- No UI play buttons / episode rows (ADR-046 builds the episode list and calls
  into the player). This slice makes the player able to play a podcast item.
- No downloads (ADR-043); the resolver returns the enclosure URL for now.
- No speed/skip transport controls (ADR-047); those are UI over existing
  `setRate` / `seek`.
...
protocol from `ADR-037-podcasts.md`. It mirrors `SubsonicStreamResolving.swift`
...
6. **Mark played on completion**: in `handleTrackEnded` (natural end) and the
   ...
   auto-marks when a position write lands within the completion tail, per ADR-041;
...
The UI (ADR-046) constructs the `QueueItem`. Define the construction contract
...
- A streamed enclosure is a finite static file with `Content-Length` and HTTP
  ...
  episode will be best-effort. This is the strongest argument for ADR-043
...
- **markPlayed idempotency.** Both the natural-end path here and the
  near-end-position path in ADR-041 can fire; both are idempotent upserts, so a
...
- **Speed (rate) and pitch.** ADR-047 adds a speed control via the existing
...
ADR-046's episode rows build the podcast `QueueItem` per the field contract
above and call the existing `QueuePlayer` play APIs. ADR-047 adds the
podcast Now-Playing rendering and the speed/skip controls over `setRate` /
`seek`. ADR-043's downloads make `resolver.audioURL` return a local file when

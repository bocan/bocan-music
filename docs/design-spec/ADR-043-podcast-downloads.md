# ADR-043: Podcasts - Episode downloads and offline playback (enhancement)

> Depends on: `ADR-037-podcasts.md`, `ADR-038-podcast-persistence.md`,
> `ADR-041-podcast-subscriptions.md`. **Optional / post-MVP.** Streaming works without
...
> already prefers a downloaded file (ADR-041).
>
...
downloaded file when one exists (already wired in ADR-041's `audioURL`).

## Non-goals

- No download of show artwork (that is `PodcastArtworkCache`, ADR-041).
...
- `podcasts.auto_download` (already in the schema) gates it. When a `refresh`
  (ADR-041) discovers new episodes for an auto-download show, enqueue the
...
- A global "downloads" budget setting (default e.g. 5 GB) shown in the Podcasts
  settings pane (ADR-047). When exceeded, evict **played + downloaded**
...
No player change needed: `PodcastService.audioURL` (ADR-041) already returns
...
resume even with no network. Confirm `resumePosition` + seek-on-load (ADR-042)
...
- `unsubscribe` (ADR-041) deletes the show's download directory (cross-check
...
ADR-046's episode rows show the download badge/affordance and call
`download` / `removeDownload`. ADR-047's settings pane surfaces the storage
budget, auto-download N, and "clear downloads". The player (ADR-042) is unchanged;

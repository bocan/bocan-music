# ADR-044: Podcasts UI - Sidebar item, subscribed grid, Add bar, UI seams

> Depends on: `ADR-037-podcasts.md`, `ADR-038-podcast-persistence.md` (read models),
> `ADR-041-podcast-subscriptions.md` (the `PodcastService` the App wires behind the
...
title + author). Clicking a show navigates to its episode list (ADR-046).
...
public protocol PodcastActions: Sendable {
    ...
    // Episode-level actions used by ADR-046; declared here so the seam is one type.
    ...
    func download(podcastID: Int64, guid: String) async         // no-op if ADR-043 not built
...
seam is declared in ADR-045 (`PodcastSearchProviding`); keep it separate so
...
the podcast `QueueItem` (per the field contract in ADR-042) and hand it to
...
`// MARK: - ADR-037 (Podcasts)` section. **Do not** confuse these with the
...
public final class PodcastsViewModel: ObservableObject {
    ...
    @Published public private(set) var episodes: [EpisodeListItem] = []     // for the open show (ADR-046)
...
public struct PodcastsHomeView: View {
    ...
    public var body: some View {
        VStack(spacing: 0) {
            PodcastAddBar(vm: vm, library: library)        // ADR-045 owns the search/detail behind it
            Divider()
            Group {
...
add-by-URL parsing are ADR-045 (which reads `vm.addBarText` and presents
...
- Submit (Return) and live typing both drive search (debounced in ADR-045).
- When the text parses as an http(s) URL, ADR-045 shows an "Add this feed" row at the
  top of the results.

ADR-045 attaches the results presentation to this bar (a `.popover` or an
overlay results list). This slice ships the bar with the text binding and the
empty/placeholder results container so ADR-045 can fill it.
...
case let .podcastShow(id):
    PodcastShowView(vm: self.vm.podcasts, library: self.vm, podcastID: id)   // ADR-046
```

(`PodcastShowView` is a stub here that ADR-046 fills; ship a minimal placeholder so
...
`play`) + the download manager (for `download`/`removeDownload`, no-ops if ADR-043 is not built). Inject both into `LibraryViewModel`. Start the
`FeedRefreshScheduler` (ADR-041) after launch.
...
struct AppPodcastActions: PodcastActions {
    ...
    let downloads: EpisodeDownloadManager?      // ADR-043; nil otherwise
    // subscribe/unsubscribe/refresh/... forward to service
    // play(episode:podcast:) builds the podcast QueueItem (ADR-042 contract) and calls player
}
```
...
ADR-045 fills the Add bar with live search results (source badges) and the
detail + Subscribe flow. ADR-046 fills `PodcastShowView` with the episode
table and reads `vm.episodes`. ADR-047 adds the podcast Now-Playing rendering

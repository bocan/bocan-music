# ADR-045: Podcasts UI - Search results (source badges), detail, Subscribe

> Depends on: `ADR-037-podcasts.md`, `ADR-040-podcast-search.md` (the search service),
> `ADR-039-podcast-feeds.md` (feed parsing for detail), `ADR-041-podcast-subscriptions.md`
> (subscribe), `ADR-044-ui-podcasts-home.md` (the Add bar that hosts this).
...
Turn the Add bar (ADR-044) into a working discovery surface: as the user
...
Extend `PodcastsViewModel` (ADR-044) with search state:
...
Attach to the Add bar (ADR-044) as an overlay/popover list that appears while
...
- Description (feed content; render as sanitized rich text, see ADR-046's HTML
...
- "Recent Episodes": a compact list of `episodePreview` (title, date, duration).
  ...
  ADR-046). Optionally allow "Play latest" which subscribes-then-plays, but MVP can
...
ADR-046 builds the episode list the user reaches after Subscribe (or via the
grid). ADR-047 may add "Play latest" / subscribe-and-play niceties and the

# ADR-046: Podcasts UI - Episode list (date, duration, progress indicator), show notes

> Depends on: `ADR-037-podcasts.md`, `ADR-038-podcast-persistence.md`
> (`EpisodeListItem` + observation), `ADR-042-podcast-playback.md` (the podcast
> `QueueItem` contract + player), `ADR-044-ui-podcasts-home.md`
...
Fills the stub from ADR-044. Routed from `ContentPane` for
`.podcastShow(id)`.
...
`vm.loadShow(id)` (ADR-044) fetches the show and starts
...
| (Download) | download badge / button | only meaningful with ADR-043; show always-empty otherwise |
...
`PodcastPlayback.completionTailSeconds` constant (ADR-041) so "played" agrees
...
ADR-044/ADR-042). The App builds the podcast `QueueItem` per the field contract
in ADR-042 (`title` = episode, `artistName` = show, source
...
`actions` from ADR-044 cover these:

- **Play** / **Play Next**.
- **Mark as Played** / **Mark as Unplayed** (-> `actions.markPlayed` /
  `markUnplayed`; the indicator flips live).
- **Download** / **Remove Download** (ADR-043; show but disable/hide cleanly
...
- [ ] Double-click / Return plays and **resumes** the episode from its saved
      position (the resume itself is ADR-042; verify end-to-end).
...
- **Live indicators come for free from the join observation** (ADR-038's
...
ADR-047 renders the playing episode in Now Playing's podcast mode (show art,

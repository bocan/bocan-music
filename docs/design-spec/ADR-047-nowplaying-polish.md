# ADR-047: Podcasts - Now Playing podcast mode, transport, settings, docs

> Depends on: `ADR-037-podcasts.md`, `ADR-042-podcast-playback.md` (the `.podcast`
> source + podcast Now Playing metadata; note there is no `.podcast` media type,
> so detect podcasts via `PlayableSource.podcast`),
> `ADR-044-ui-podcasts-home.md` (seams + view model),
> `ADR-046-ui-episodes.md` (the play surface). ADR-043 informs the settings
...
- `artist` = show name (already carried by `QueueItem.artistName`, per the ADR-042 field contract, so the artist slot shows the show with **no new
  plumbing**).
...
- Wire these to the `MPRemoteCommandCenter` skip commands too
  ...
  `PlayableSource.podcast`, the same signal ADR-042 uses; there is no
...
`FeedRefreshScheduler` (ADR-041); the download settings feed ADR-043 (hide
the download rows cleanly if ADR-043 is not built); skip/speed/mark-played feed the
...
- Every string added across ADR-044 to ADR-047 (sidebar label, Add bar prompt,
...
- The status indicator already differs by shape (ADR-046); confirm VoiceOver
...
- Source badges carry the source(s) in their label (ADR-045).
...
- `NOTICES.md`: ensure FeedKit (ADR-039) is listed with its license. Note the
  use of the Podcast Index and Apple iTunes Search APIs (attribution as their
  terms require).
- `docs/design-spec/README.md`: the ADR-037 rows are added (this spec set does
...
- **Settings**: values persist and round-trip; the refresh interval reaches the
  scheduler; download rows hidden when ADR-043 absent.
...
With this slice, ADR-037 is done:

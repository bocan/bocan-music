# ADR-050: Supplementary podcast: namespace parser (funding + chapters)

> Depends on: `ADR-048-feedkit-upgrade.md` (FeedKit on 10.4.0; `FeedParser`
> already rewritten to the 10.x API, `podcastGUID` field and column landed) and
> `ADR-049-podcast-features.md` (the contract for the ADR-049 slices). Read
> `_standards.md` and `ADR-037-podcasts.md` first, then those two, then this.
>
> This is ADR-050, the Enabler. It is a hard prerequisite for ADR-052
> (funding affordance) and ADR-053 (chapters list). Build it first.
...
- `podcast:person`, `podcast:soundbite`, `podcast:value`, `podcast:location`, and
  every other Podcasting 2.0 tag (deferred research, per the ADR-049 overview).
- Any UI. Surfacing funding is ADR-052; rendering chapters is ADR-053.
...
- Fetching the chapters JSON document. `chapters_url` is a URL only here; the
  on-demand fetch and parse of the chapters file is ADR-053.
...
`ADR-037-podcasts.md`). Only the funding label is new. The next free migration
...
- Only keep `http` / `https` URLs (treat feed URLs as untrusted, per the ADR-049
...
- **funding_text is feed content.** Render it verbatim, never through `L10n`. The
  ...
  its confirmation) belongs to ADR-052, in the UI module.
...
columns are populated on subscribe and refresh. ADR-052 reads
`funding_url` + `funding_text` to render the "Support this show" affordance (with
its confirmation dialog and host display); ADR-053 reads `chapters_url` to

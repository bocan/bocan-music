# ADR-052: Support this show (funding link with confirmation)

> Depends on: `ADR-050-namespace-supplement.md` (the supplementary `podcast:`
...
> `ADR-049-podcast-features.md` (the contract) and `ADR-046-ui-episodes.md`
> (the `PodcastShowView` header + `moreMenu` this slice extends).
>
> Read `_standards.md` and `ADR-037-podcasts.md` first, then the contract.
...
(`ADR-049-podcast-features.md`, "Sandbox"), and routing this through a new
...
- **ADR-050 is the prerequisite.** `Podcast.fundingText` does not exist until
  `ADR-050` lands; if this slice is built first, `fundingLink` can read
  `fundingURL` alone and treat the label as always-nil, but prefer ordering after
  a so the verbatim-label path is real.

## Handoff

With this slice, parsed funding URLs (from ADR-050) become a user-facing,
...
remaining sub-ADRs (`ADR-053` to `ADR-058`) are unaffected. If a future

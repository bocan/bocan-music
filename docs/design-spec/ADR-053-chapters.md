# ADR-053: Chapters in Now Playing
...
> chapters URL to fetch). Read `_standards.md`, `ADR-037-podcasts.md`, and the
> contract `ADR-049-podcast-features.md` first. Builds on ADR-042 playback
> (`PlayableSource.podcast`, `QueuePlayer`) and ADR-047 Now Playing podcast mode.
...
- **`chapters_url` comes from ADR-050.** Before the supplement runs the column is
...
future ADR wants offline chapters, the on-disk `ChaptersCache` is the place, or a table
can be added then.

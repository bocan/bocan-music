# ADR-054: Continue Listening rail

> Depends on: `ADR-049-podcast-features.md` (the contract). Builds on
> ADR-038 (`podcast_episode_state` + repositories), ADR-042 (the
> `PodcastEpisodeResolving` resolver, which already seeks to the saved position),
> and ADR-044 (the Podcasts home view + UI seam protocols). Read
> `_standards.md`, `ADR-037-podcasts.md`, and the contract above first.
...
- No swipe-to-dismiss. An episode leaves the rail only when marked played or
  unplayed.
- No change to position write-back, resume, or `markPlayed` (ADR-042). This
...
`PodcastLibraryDataSource`, and resuming through the ADR-042 resolver. It
shares the state table with ADR-051 (transcript cleanup) and ADR-055

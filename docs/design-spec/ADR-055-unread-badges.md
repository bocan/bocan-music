# ADR-055: Unread badges and Mark all as played

> Depends on: `ADR-049-podcast-features.md` (the contract; read "Shared
> semantics" -> "Unread count" and the cleanup interaction),
> `ADR-044-ui-podcasts-home.md` (subscribed grid, `PodcastCell`), and
> `ADR-051-transcripts.md` (the 30-day cleanup clock this slice can start).
> Read `_standards.md` and `ADR-037-podcasts.md` first.
...
- No per-episode unread toggle in the grid (per-episode mark-played already
  exists in the episode list from ADR-046; this slice is show-level only).
...
which starts the 30-day transcript cleanup clock from ADR-051 for each one.**
...
- **Cleanup-clock interaction.** Mark-all sets `play_state = 'played'` and stamps
  `completed_at`/`last_played_at` for every episode, exactly what arms ADR-051's 30-day transcript sweep. By design and cross-referenced in both specs; add
...
state table ADR-051 (cleanup) and ADR-054 (continue listening) read, so

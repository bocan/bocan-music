# ADR-056: OPML import / export

> Depends on: `ADR-049-podcast-features.md` (the contract for the ADR-049
> slices) and the whole ADR-037 feature set (`PodcastService.subscribe`,
...
> `ADR-037-podcasts.md` first, then the ADR-049 contract, then this. Independent
> of the other ADR-049 slices; can land in any order.
...
file-picker + partial-failure summary idiom. If a later ADR adds folder/group

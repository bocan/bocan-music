# ADR-057: Per-show settings (speed, sort, retention)

> Depends on: `ADR-049-podcast-features.md` (the contract), `ADR-046-ui-episodes.md`
> (the episode table), `ADR-047-nowplaying-polish.md` (the speed control and the global
> podcast-speed default). Read `_standards.md` and `ADR-037-podcasts.md` first.
...
- Retention by age or played-state (count only here; the transcript 30-day clock from
  ADR-051 is a separate concern, untouched).
...
is **M025**. If an earlier ADR-049 sub-ADR took 025, use the next free integer and adjust
...
(ADR-051) or unread counts (ADR-055).

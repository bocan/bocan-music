# ADR-058: podcast:guid identity and de-duplication

> Depends on: `ADR-048-feedkit-upgrade.md` (FeedKit on 10.4.0; the
...
> `ADR-049-podcast-features.md` (the contract) and `ADR-037-podcasts.md`
...
ADR-048) to recognize the same show across feed-URL changes (host
...
- No cross-device subscription sync (still a ADR-037 non-goal).
...
> Migration numbering: M024 is the highest registered today; ADR-050
> (`ADR-050`) claims **M025** for `funding_text`. Per the ADR-049 build order
...
column is `NULL` for every pre-ADR-048 row until its next refresh), and SQLite
...
/// See `docs/design-spec/ADR-058-guid-identity.md`.
///
...
`podcast:guid` semantics were settled in ADR-048; no new external API is
...
- **Null guid is the common case, not the exception.** Every row last refreshed
  before ADR-048 has `podcast_guid == NULL`, and many real feeds never publish
...
creating a duplicate, keyed on the `podcast:guid` parsed and stored since ADR-048, with episode state preserved because `podcast_id` is stable. `feed_url`
...
without changing this logic. This is the last lettered slice of ADR-049;
with it, `podcast:guid` backs identity across feed-URL changes as the ADR-049

# ADR-060: Phone Sync Server - Overview and Cross-ADR Contract

> Prerequisites: ADR-001 to ADR-037 complete. The module DAG, the GRDB `Database`
> actor + numbered-migration + repository pattern, the `SecurityScope` helper
> (ADR-004), the cover-art cache, the smart-playlist criteria evaluator, the
...
> the contract.** ADR-061 through ADR-069 each implement one slice of it; read
> this overview before starting any of them so the shared types, table shapes,
> endpoint behaviour, and trust model line up.
>
> **This ADR has a second binding document that lives in the sibling repo:**
...
> brief this group implements; this ADR-060 group is the Mac-repo expansion of
> it.
...
which needs the Android pairing + sync engine (its ADR-0ADR-004 and 03) runnable;
...
- **Separate identity from the (unbuilt) ADR-034 remote-control server.** ADR-034 is a control plane with its own protocol; it is not implemented in this
  ...
  store. Do not merge them. If ADR-034 ever lands, the Bonjour/TLS/HTTP
...
| `pairing-vectors.json` | `core/sync/src/test/resources/fixtures/pairing-vectors.json` | copied byte-identical into `Modules/SyncServer/Tests/SyncServerTests/Fixtures/` (ADR-061) | generated; commit on both sides |
| `manifest-small.json` | `core/persistence/src/test/resources/fixtures/manifest-small.json` | copied into `Modules/SyncServer/Tests/SyncServerTests/Fixtures/` (ADR-065) | committed |
...
- The golden manifest is produced by the Android side from its DTOs. The Mac
  ...
  ADR-065 owns that parity test.

## How ADR-060 is split
...
|------|-------|-------------------|
| `ADR-060-phone-sync.md` | This contract (read first) | - |
| `ADR-061-pairing-code.md` | `PairingCode` + golden vectors (test-first) | SyncServer |
| `ADR-062-identity-trust.md` | `ServerIdentity` (Keychain P-256) + `TrustedDevices` (migration) | SyncServer, Persistence |
| `ADR-063-http-listener.md` | `HttpConnection` + `Router` + `NWListener`/TLS verify block + `/v1/ping` | SyncServer |
| `ADR-064-pairing-ceremony.md` | `PairingSession` + `/v1/pair/*` + pairing-mode | SyncServer |
| `ADR-065-manifest.md` | `SyncProfile` + `ManifestBuilder` + generation counter + change observer | SyncServer, Persistence |
| `ADR-066-file-serving.md` | `FileServing` (Range/If-Match) + artwork/lyrics/chapters endpoints | SyncServer |
| `ADR-067-lifecycle-bonjour.md` | `SyncServer` actor lifecycle + Bonjour + app wiring + sleep/wake | SyncServer, App |
| `ADR-068-settings-ui.md` | Settings pane + pairing sheet, localized, snapshot-tested | UI, App |
| `ADR-069-docs-e2e.md` | README + website + cross-repo end-to-end + acceptance sweep | docs |

MVP is ADR-061 through ADR-068 (a working, paired, end-to-end sync). ADR-069 is the
documentation and cross-repo verification sweep that makes this ADR "done" per
the standards, and cannot fully close until the Android client is runnable.

The cross-repo compatibility proof lands early on purpose: ADR-061 makes the
Mac reproduce the Android pairing golden vectors before either side talks to the
other, and ADR-065 makes the Mac reproduce the golden manifest. If those two
...
- Merging with, or reusing the identity/port/trust store of, the ADR-034
...
Wiring to update (do these in ADR-062 when the module first exists, and again
in ADR-067/ADR-068 as the edges appear):
...
Owned by `ServerIdentity` (ADR-062). This is a **separate** identity from any
future ADR-034 remote-control identity.
...
- `POST /v1/pair/start`: phone sends `{protocolVersion, deviceName, noncePhone}`;
  ...
  fingerprint.
- Both sides compute the code from the frozen formula in section 4 (see ADR-061 for the exact bytes and `PairingCode`).
...
```swift
// SyncServer / Manifest (ADR-065). Shape is fixed by sync-protocol.md s7.
...
// nested ReplayGain / Clip DTOs are defined field-for-field in ADR-065,
// exactly per sync-protocol.md section 7. The clip DTO is
// { sourceTrackId, startMs, endMs }.
```

```swift
// SyncServer / Manifest (ADR-065). What the phone is allowed to see.
...
Implementation (ADR-065): a persisted counter in a new `sync_meta` table
...
When all of ADR-060 lands:

- `SyncServer` is the home for any future LAN sync work. Any later "ADR-034"
...
- Cross-repo integration is proven by the shared fixtures (pairing vectors,
  ...
  repos' PRs (ADR-069), consumed by the Android ADR-0ADR-004 and 03.

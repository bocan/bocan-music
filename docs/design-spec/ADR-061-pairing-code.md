# ADR-061: PairingCode + Golden Vectors

> Depends on: `ADR-060-phone-sync.md` (read it first). No other prerequisites:
...
compatible before either side opens a socket. Nothing else in ADR-060 should be
...
Defer the `_standards.md` / `CLAUDE.md` DAG-table edits to ADR-062, where the
...
ADR-062 (identity computes the `fpMac` this consumes) and ADR-064 (the ceremony

# ADR-062: ServerIdentity (Keychain) + TrustedDevices (Persistence)

> Depends on: `ADR-060-phone-sync.md`, `ADR-061-pairing-code.md`.
>
...
- **SecIdentity**: assembled from the `SecKey` + `SecCertificate`
  ...
  back). Exposed for `TLSOptions` (ADR-063).
...
- Service string: `io.cloudcauldron.bocan.sync` (new, distinct from
  `.subsonic` / `.scrobble`, and distinct from any future ADR-034 remote
  identity).
...
`sync_meta` and `sync_profile` are consumed by ADR-065 (generation, serverId,
...
or defer them to ADR-065. Prefer adding all three tables in the one migration (a
migration is append-only once shipped) and leaving the sync_meta/profile
repositories for ADR-065.
...
so the TLS verify block (ADR-063) does not hit the database per handshake:
...
- `Modules/SyncServer/Package.swift`: add `Persistence` (and `Observability`,
  ...
  slices that first need them (ADR-065/ADR-066); do not add them speculatively.
- `_standards.md`: add the `SyncServer` row to the "Current internal-module
  ...
  Library/Podcasts edges arrive in ADR-065/ADR-066, or add incrementally). Add
  `SyncServer` to the `UI` row when ADR-068 lands.
...
- `project.yml`: (already added the package in ADR-061) no change unless the target
...
- **`ServerIdentityTests`**: fingerprint is 64 lowercase hex chars; two `load()`
  ...
  loopback TLS handshake (shares the `TestIdentity` helper with ADR-063). Use a test
...
- **`TrustedDevicesTests`**: `isTrusted` reflects an inserted device after
  ...
  observation wiring, mirroring the revoke-takes-effect acceptance test in ADR-063).
...
- [x] The identity's `SecIdentity` completes a loopback TLS handshake as the
      server (verified with the ADR-063 harness).
...
ADR-063 consumes `ServerIdentity.Loaded.secIdentity` for the TLS local
identity and `TrustedDevices.isTrusted` for the verify block. ADR-064 calls
`TrustedDevices.trust(...)` on a successful pairing confirm. ADR-065 fills in

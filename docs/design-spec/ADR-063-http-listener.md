# ADR-063: HTTP Listener + Router + TLS Verify Block

> Depends on: `ADR-060-phone-sync.md`, `ADR-061-pairing-code.md`,
> `ADR-062-identity-trust.md` (needs `ServerIdentity` for the TLS local
...
- **Responses always carry `Content-Length`.** No chunked responses. (Gzip for
  the manifest is added in ADR-065 and still sets `Content-Length` on the
  compressed body.)
...
paths here; see ADR-066). Property-test it with truncated, oversized, and
malformed inputs.
...
> not a fresh DB round-trip per handshake, to keep handshakes cheap. ADR-062
...
| POST | `/v1/pair/start` | ADR-064 |
| POST | `/v1/pair/confirm` | ADR-064 |
| GET | `/v1/manifest` | ADR-065 |
| GET | `/v1/file/track/{trackId}` | ADR-066 |
| GET | `/v1/file/episode/{episodeId}` | ADR-066 |
| GET | `/v1/artwork/{hash}` | ADR-066 |
| GET | `/v1/lyrics/{trackId}` | ADR-066 |
| GET | `/v1/chapters/{episodeId}` | ADR-066 |
...
`PingProviding` closure) since `sync_meta` wiring lands in ADR-065; keep the handler
...
shared helper code with ADR-062's tests; factor it into a `TestIdentity`
helper under `Tests/`.
...
ADR-064 registers `/v1/pair/*` on this `Router` and reads the recorded peer
cert from `ConnectionContext`. ADR-065 and ADR-066 register the paired-only
endpoints. ADR-067 owns the `SyncServer` actor that constructs the listener

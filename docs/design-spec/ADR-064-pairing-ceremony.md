# ADR-064: Pairing Ceremony (server side)

> Depends on: `ADR-060-phone-sync.md`, `ADR-061-pairing-code.md` (the math),
> `ADR-062-identity-trust.md` (`ServerIdentity` fingerprint + `TrustedDevices`),
> `ADR-063-http-listener.md` (`Router`, `ConnectionContext`, TLS verify block).
>
...
ADR-068; this slice exposes the seam it drives.
...
2. Read the **peer certificate from the `ConnectionContext`** (recorded by the
   verify block in ADR-063), never from the JSON, and compute `fpPhone`.
...
`serverId` comes from `sync_meta` (ADR-065 owns minting/storing it; until then
...
(harmless due to the code check + human confirm, but noisy and wrong). ADR-067
...
Implemented by the settings pairing sheet in ADR-068; a test double drives the
...
- **`PairingCeremonyTests`** (loopback TLS from ADR-063): full happy path over a real
  ...
  **subsequent** connection with the same client cert is admitted outside pairing
  mode (proves trust persisted). A revoke (ADR-062) then blocks the next
  connection.
...
ADR-067 reads `isPairingMode` for the TXT `pm` field and calls `arm()` /
`cancel()` from the settings toggle + "Pair a phone" button. ADR-068
implements `PairingUIBridge` as the real sheet. ADR-065/ADR-066 endpoints are

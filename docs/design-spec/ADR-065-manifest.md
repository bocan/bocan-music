# ADR-065: SyncProfile + ManifestBuilder + Generation

> Depends on: `ADR-060-phone-sync.md`, `ADR-062-identity-trust.md` (the M031
> tables), `ADR-063-http-listener.md` (the `Router` + `/v1/ping`).
...
pairing vectors in ADR-061): the Mac must produce a manifest whose values match the
...
4. **`lyricsHash` is computed, not stored.** There is no lyrics column on
   ...
   (ADR-066 serves the same one), serialize it canonically (LRC for synced,
...
ADR-066 serves the bytes the manifest describes (tracks/episodes/artwork/
lyrics/chapters) and relies on `sha256`/`relPath` being exactly what the manifest
advertised (`If-Match` uses the manifest `sha256`). ADR-068's profile editor

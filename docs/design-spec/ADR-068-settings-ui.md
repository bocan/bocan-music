# ADR-068: Settings Pane + Pairing Sheet

> Depends on: `ADR-060-phone-sync.md`, `ADR-064-pairing-ceremony.md` (the
> `PairingUIBridge` seam + coordinator), `ADR-065-manifest.md`
> (`SyncProfile` + repository, for the profile editor + size estimate),
> `ADR-067-lifecycle-bonjour.md` (`SyncServer` start/stop + enable toggle).
> Touches **UI** and **App**.
>
...
user-facing surface of ADR-060; it implements the `PairingUIBridge` the ceremony
drives.
...
2. **Sync profile**: a picker between **Everything** and **Choose playlists**;
   ...
   (which bumps `generation`, per ADR-065).
3. **Paired devices**: a `List` of `TrustedDevice` rows (device name, paired
   ...
   (ADR-062/ADR-063). Empty state: "No paired phones yet."
...
ADR-069 records the manual real-device end-to-end run driven from this UI
(pair a phone, pick a profile, sync), and finalizes docs. After ADR-068 the feature

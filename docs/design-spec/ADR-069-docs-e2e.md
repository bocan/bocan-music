# ADR-069: Docs + Cross-Repo End-to-End + Acceptance Sweep

> Depends on: all of ADR-061 through ADR-068 landed and green. This slice is the "done"
> gate for ADR-060 and the only one whose full completion needs the **Android**
> client runnable (its ADR-003 pairing + 03 sync engine).
>
...
Close out the Phone Sync series: user-facing docs (README + website), the cross-repo integration
...
- Cross-link: the Android repo's `phase-mac-1-sync-server.md` and this ADR-060
...
1. **Pairing golden vectors** (`pairing-vectors.json`): byte-identical and green
   in both repos (Mac: ADR-061 `PairingCodeTests`; Android: its
...
2. **Golden manifest** (`manifest-small.json`): the Mac `ManifestBuilder`
   (ADR-065) produces a value-identical manifest for the shared fixture
...
3. **Manual end-to-end run** (needs Android ADR-003 + 03 runnable): pair a real
   ...
   merge time, land ADR-061 through ADR-068, mark this box "pending Android 02/03," and
...
- [ ] **Pending the manual device run** (see the verification record below):
      ...
      end to end. (ADR-064, ADR-065, ADR-066, ADR-067, ADR-068; the manual run above.) Every
...
- [x] The manifest validates against `sync-protocol.md` and the shared fixtures;
      the Android `SyncApplier` accepts it unmodified. (ADR-065 + this slice:
      Android `SyncApplierTests` and `ManifestCodecTests` green against the
      byte-identical fixture, 2026-07-11.)
- [x] Revoke immediately blocks a paired device at the TLS layer. (ADR-062/ADR-063;
      tested in ADR-064.)
- [x] Pairing golden vectors shared and green in both repos. (ADR-061 + this slice:
      `diff` byte-identical, Mac `PairingCodeTests` and Android
      `PairingCodeTests` green, 2026-07-11.)
- [x] Range resume and If-Match staleness behave per contract (tested). (ADR-066.)
- [x] Serving leaves the UI responsive and writes nothing to the library.
      (Off-main handlers proven in ADR-063/ADR-066; read-only proven by the grep in the
      verification record below; the concurrent-download responsiveness test in
      ADR-066 is green. The at-scale 10 GB soak against a real phone is folded into
      the pending manual run.)
- [x] All new user-facing strings localized; `make pseudolocale` green; snapshot
      tests updated. (ADR-068.)
...
- [x] Every acceptance box across ADR-060 through ADR-069 ticked, except the two boxes
...
- [x] CI green on main through every landed slice (latest: the ADR-068 settings
      pane commit); this slice's run triggers on push.
- [x] No `TODO(ADR-060)` left behind. (Grep over sources, specs, and build
      files: zero hits.)
...
"0 tracks": every track's `content_hash` was NULL, and the ADR-065 manifest
correctly excludes hashless tracks, so the manifest was empty. The column had
existed since M001 with the ADR-003 note "computed lazily, not on scan", but
...
Android ADR-003 (pairing) and 03 (sync engine) are landed, so the run needs
...
links its ADR. No AI attribution / trailers. No em/en dashes in commit
...
- [x] Whole-workspace `make format && make lint && make build && make
      ...
      `TODO(ADR-060)` remains.

## Handoff (series complete)
...
LAN sync work; the Android ADR-0ADR-004 and 03 consume this server; a later ADR-034

# ADR-070: Podcast Artwork over Sync

> Depends on: `ADR-065-manifest.md` (the manifest advertises artwork by content
> hash), `ADR-066-file-serving.md` (`GET /v1/artwork/{hash}` streams cover-art
> by hash through the DB), `ADR-041-podcast-subscriptions.md` / `ADR-043-podcast-downloads.md`
...
ADR gives podcast art a content hash, advertises it, and teaches the existing
...
- No sync-protocol.md change. The `Podcast.artworkHash` field already exists and
  is documented as optional; this ADR only starts populating it.
...
and served from that path. The cardinal rule from ADR-066 still holds verbatim: the
...
- **Path safety**: the podcast fallback resolves only via the DB column; a
  request-supplied path can never reach it (reuse the ADR-066 traversal test).
...
After this ADR the manifest carries podcast artwork hashes and the artwork
endpoint serves them, so the existing Android sync pipeline displays show art with
no further phone work. A future ADR could add per-episode artwork (a new
`ManifestEpisode.artworkHash`) if feeds that vary art per episode become worth
supporting; this ADR intentionally stops at show-level art to match the v1 UI.

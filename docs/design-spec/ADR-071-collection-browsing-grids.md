# ADR-071: Collection Browsing Grids - Overview and Cross-Slice Contract

> Prerequisites: ADR-001 to ADR-060 complete. The module DAG, the GRDB `Database`
> actor + repository pattern, `AlbumsGridView` (ADR-005), the playlist mosaic
> generator (ADR-008), the `L10n` localization workflow (#314), and the
...
> the contract.** Slices ADR-072 through ADR-074 each implement one piece of it; read
...
ADR gives each of them a second, visual browsing mode: an adaptive grid of
...
A second, smaller piece (slice ADR-074) addresses the ticket author's literal
...
|-------------------------|----------------------|---------|-------|
| `artists.viewMode`      | `CollectionViewMode` | `.list` | ADR-072  |
| `genres.viewMode`       | `CollectionViewMode` | `.list` | ADR-073  |
| `composers.viewMode`    | `CollectionViewMode` | `.list` | ADR-073  |
| `genres.detailMode`     | `CollectionDetailMode` (`songs` / `albums`) | `.songs` | ADR-074 |
| `composers.detailMode`  | `CollectionDetailMode`                      | `.songs` | ADR-074 |
...
| Slice | File | Delivers |
|-------|------|----------|
| ADR-072 | [ADR-072-artists-grid.md](ADR-072-artists-grid.md) | Shared infrastructure (`CollectionViewMode`, `CoverMosaicGenerator`, card + grid components, artist cover query) and the Artists grid mode with toggle + persistence |
| ADR-073 | [ADR-073-genres-composers-grid.md](ADR-073-genres-composers-grid.md) | Genre + composer card queries and grid modes; splits `GenresComposersView.swift` if the lint ceiling requires |
| ADR-074 | [ADR-074-view-menu-destination-albums.md](ADR-074-view-menu-destination-albums.md) | View-menu integration; Songs / Albums destination switch for genre and composer detail; README + website docs |
...
- No album-grid mode for Recently Added / Recently Played / Most Played smart
  ...
  Note it as a possible follow-up in the ADR-074 handoff; do not build it.
...
- **`App/` has no String Catalog.** Existing menu titles in `BocanCommands`
  are grandfathered bare literals; slice ADR-074 follows that file's existing
  convention for the two new items and goes no further. Menu-bar localization
  is a known pre-existing gap; do not try to solve it inside this ADR.
...
- **`AlbumInteractiveCell` in `AlbumsGridView.swift` is private.** Do not
  ...
  nice-to-have, not a requirement, in this ADR.
...
- [ ] README + website updated (slice ADR-074). Issue #363 closed by the release.

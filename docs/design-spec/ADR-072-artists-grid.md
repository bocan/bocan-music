# ADR-072: Shared Card Infrastructure + Artists Grid

> Depends on: `ADR-071-collection-browsing-grids.md` (read it first; it is the contract).
...
Genres/composers (ADR-073). View menu and destination modes (ADR-074). Keyboard
...
All in `ADR-071-collection-browsing-grids.md`; produce them verbatim where specified.
...
ADR-073 consumes `CollectionCard`, `CollectionCardGrid`, `CollectionViewMode`,

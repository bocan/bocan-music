# ADR-073: Genres and Composers Grids

> Depends on: `ADR-071-collection-browsing-grids.md` (the contract) and a committed,
> gates-green ADR-072 (it reuses every component ADR-072 built).
...
View menu, destination Songs / Albums modes, docs (all ADR-074).
...
5. **Localization**: per-section accessibility hints ("Opens this genre's
   songs", "Opens this composer's songs"); reuse everything else from ADR-072.
   `make pseudolocale`.
...
ADR-074 expects: all three sections have working, persisted grid modes; the

# ADR-074: View Menu + Destination Album Modes + Docs

> Depends on: `ADR-071-collection-browsing-grids.md` (the contract) and committed,
> gates-green ADR-072 and ADR-073.
...
- The Songs / Albums control is a segmented `Picker` in the destination's
  ...
  accessibility labels, same conventions as the ADR-072 toggle.
...
issue if the ticket author still wants them. ADR-071 is then complete and
2.3.0-ready.

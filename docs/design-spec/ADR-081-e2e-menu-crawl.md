# ADR-081: E2E Menu Bar Crawl

> Prerequisites: ADR-079 to ADR-080. The menu bar is wired in `App/BocanCommands`
...
- Context menus inside views (ADR-082 owns those with their surfaces).
...
- E2E launches share the container's real `UserDefaults` (isolation is a
  ADR-083 decision), so preference-gated items ("Fetch Lyrics from
...
ADR-082 reuses the manifest interpreter pattern for per-surface control

# ADR-085: E2E Hermetic Network (Radio and Podcast Servers)

> Prerequisites: ADR-079 to ADR-082. Nightly runs must never touch the internet,
> yet radio and podcasts are network features; this ADR gives them
...
> biggest lift and can become a later ADR if wanted).
>
...
ADR-086 schedules these suites on both tiers (they are CPU-cheap) and

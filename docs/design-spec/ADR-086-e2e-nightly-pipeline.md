# ADR-086: E2E Local Pre-Release Checks (CI Deferred)

> Prerequisites: ADR-079 to ADR-085 (any subset already landed benefits
> immediately; this ADR can start once 28 exists and grow).
>
...
sanity check, sitting alongside the full `make test-e2e` board from ADR-085.

## Status: CI scope dropped

The original plan for this ADR was a two-tier GitHub Actions pipeline: a
...
`make test-e2e` (ADR-085) already runs the full `BocanUITests` board
...
The smoke subset: `FoundationJourneys` (ADR-079 journeys), the menu
...
deliberately excludes the reconnect pair (the ADR-085 flake watch, slow

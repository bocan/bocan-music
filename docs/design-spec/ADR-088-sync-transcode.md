...
- **The prepare window.** The coordinator never holds more than a fixed budget of un-served artifact bytes (default 5 GB). When the window is full it pauses encoding until the phone drains it. A sync therefore interleaves: encode ahead, phone pulls, bytes released, encode continues. If the phone is offline mid-pass, preparation simply parks at the window and resumes when the phone returns; nothing accumulates beyond the budget.

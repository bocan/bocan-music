...
| `Sources/UI/{Utility,Accessibility,Components,Localization}` + top-level files | ~7 | ~1.0k | Helpers, the ADR-071 `Components/`. |
...
- **View-model `load()` patterns.** Several view models repeat
  ...
  `AlbumsViewModel` / `ArtistsViewModel` / tracks / podcasts loaders (ADR-071
...
- **Scroll-restore (#349) pattern.** The offset-snapshot + restore appears in the
  albums grid, artists, and the ADR-071 collection grids. It is already partly
  shared via `CollectionCardGrid`; check the remaining hand-rolled copies for a
  common modifier.
- **Confirm the ADR-071 `Components/`** (`CollectionCard`, `CollectionCardGrid`,

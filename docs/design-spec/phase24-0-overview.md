# Phase 24: Transcode Detection (Audio Provenance)

> Prerequisites: Phases 0-23 complete. `ReplayGainAnalyzer` (`AudioEngine`)
> decodes whole files offline via `AVAudioFile` for the EBU R128 pass and is
> the infrastructure precedent for this phase. The Library Summary window
...
### 24-1: `ProvenanceAnalyzer` (AudioEngine)
...
- Scope control: analyse only tracks claiming lossless (`is_lossless = 1`);
  lossy-from-lossy detection is explicitly out of scope for this phase.
...
- Stretch, not required for the phase: a small spectrogram thumbnail per
...
- No lossy-from-lossy detection (192k MP3 re-encoded to 320k) in this phase.

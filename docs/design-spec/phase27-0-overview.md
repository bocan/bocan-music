...
> titles are an engine seam away, not a proxy away. Phase 19's per-server
> "Internet Radio" rows and `SubsonicInternetRadioView` stay as they are;
> this phase adds the local, user-curated station catalog beside them.
> Closes #376.
>
...
- Migration `M036_RadioStations`, id `036_radio_stations`, registered after
  ...
  columns filled in by 27-5 on successful connect: `genre`,
  `station_description`, `last_codec`, `last_bitrate_kbps`,
  `last_connected_at`. The profile makes the info sheet useful offline and
  gives later phases (loudness, honesty) a home without another migration.
...
- `RadioView` and view model modeled on `SubsonicInternetRadioView` (list,
  ...
  info sheet shows the 27-1 profile fields when present. The list observes
...
## Future phases (parked, vetted against the codebase)

Ideas from the radio-detection survey that are real and worth a phase of
...
- **Stream honesty**: `ProvenanceAnalyzer` / `SpectralShelf` (phase 24) on
  live PCM: "claims 320k, spectral shelf says 128k source". Suspected,
  never accused, same stance as phase 24.
- **HLS stations and DVR windows**: FFmpeg's hls demuxer sits behind the
  ...
  unexplored. Treat as its own phase if demand appears.
...
- Station names from `#EXTINF` are frequently junk ("0," prefixes, empty
  titles). The host-name fallback plus the edit sheet plus 27-5's
...
- HLS master playlists pasted as station URLs are unvalidated territory in
  this phase: they may play via FFmpeg's hls demuxer, or fail oddly. The
  add sheet accepts them (see 27-3) but nothing else special-cases them.
